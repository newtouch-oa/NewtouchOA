var baseContentUrl = contextPath +  "/core/funcs/workflow/flowdesign/viewlist/setproperty"
...
function doInit(){
  ...
  if(isEdit){
    //修改
    var tabIndex = "<div style='text-align:right'>保存并转到：<input onclick='turn(1)'  class='SmallButtonW' value=\"上一步\" type=\"button\">&nbsp;<input onclick='turn()'  class='SmallButtonW' value=\"下一步\" type=\"button\"></div>";
    tabs = buildTab(jso, 'contentDiv' , "", tabIndex);
    displayEditMessage();
  }else{
    //增加
...
function turn(type) {
  ...
  if (json.rtState == '0') {
    ...
    var url = contextPath + "/core/funcs/workflow/flowdesign/viewlist/setproperty/index.jsp?flowId="+flowId + "&isList=" + isList + "&openflag=" + openflag;
    if (json.rtData) {
...
function mapadd(data) {
  ...
  for(var i = 0; i < relArray.length - 1; i++){
    var imgUrl = contextPath + "/core/funcs/workflow/flowdesign/viewlist/setproperty/img/remove.png" ;
...
function addUser(backUserHo,backUserHoDesc,backUserOp ,backUserOpDesc , par){
  ...
  var chooseType = window.showModalDialog(contextPath + "/core/funcs/workflow/flowdesign/viewlist/select/FlowUserSelect.jsp",window,"dialogLeft=480px;dialogTop=350px;dialogWidth=510px;dialogHeight=400px;status=no;resizable=no");
}
...
function multiUserSelect(userHi, userDesc){
  UserHi = $(userHi) ;
  UserDesc = $(userDesc);
  var chooseType = window.showModalDialog(contextPath + "/core/funcs/workflow/flowdesign/viewlist/select/MultiUserSelect.jsp",window,"dialogLeft=500px;dialogTop=350px;dialogWidth=500px;dialogHeight=400px;status=no;resizable=no"); 
}
...
function selPlugin(){
  var url = contextPath + "/yh/core/funcs/workflow/act/YHPluginAct/getPluginList.act" ;
  loc_y=loc_x=200;
