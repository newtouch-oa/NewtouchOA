...
function openCommentWindow(runId , flowId) {
  var url = contextPath + "/core/funcs/workflow/flowrun/list/comment.jsp?runId=" + runId + "&flowId=" + flowId;
...
function loadFlowType(){
  var url = contextPath+'/yh/core/funcs/workflow/act/YHWorkQueryAct/getFlowTypeJson.act?sortId=' + sortId;
...
function restore() {
  var par = arguments[2];
  var url = contextPath + "/yh/core/funcs/workflow/act/YHFlowManageAct/restore.act";
...
function handleWork(par){
  var url = contextPath + "/core/funcs/workflow/flowrun/list/inputform/index.jsp?" + par;
  location =  url;
}
function edit(){
  var par = arguments[2];
  var url = contextPath + "/core/funcs/workflow/flowrun/list/inputform/edit.jsp?" + par;
...
function exportZip() {
  ...
  var url = contextPath + "/yh/core/funcs/workflow/act/YHFlowExportAct/exportFlowZip.act?runIdStr=" + runIds;
  window.open(url);
}
...
function endWorkFlow() {
 ...
 var url = contextPath + "/yh/core/funcs/workflow/act/YHFlowManageAct/endWorkFlow.act";
 var json = getJsonRs(url, "runIdStr=" + runIds) ;
 if (json.rtState == '0') {
   alert(json.rtMsrg); 
   pageMgr.search();
...
function focus(event , ee, par) {
  if(window.confirm(tooltipMsg6)) {
    var url = contextPath + "/yh/core/funcs/workflow/act/YHFlowManageAct/focus.act";
    var json = getJsonRs(url, par) ;
    if (json.rtState == '0') {
      alert(json.rtMsrg); 
      pageMgr.search();
...
function calFocus(event , ee, par) {
  if(window.confirm(tooltipMsg7)) {
    var url = contextPath + "/yh/core/funcs/workflow/act/YHFlowManageAct/calFocus.act";
    var json = getJsonRs(url, par) ;
    if (json.rtState == '0') {
      alert(json.rtMsrg); 
      pageMgr.search();
