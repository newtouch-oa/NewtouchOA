    ...
    if (isTouchDevice) {
      $('content').value = $("contentTextarea").value;
	   ...
	   if (checkWords($('content').value)) {
		...
		var url = "<%=contextPath%>/yh/core/funcs/sms/act/YHSmsAct/addSmsBody.act?smsType="+<%=smsType%>;
	    var rtJson = getJsonRs(url, mergeQueryString($("smsBodyInfoForm")));
	    if (rtJson.rtState == "0") {
			bindJson2Cntrl(rtJson.rtData);
			var toId = document.getElementById("user").value;
			var url = "<%=contextPath %>/core/funcs/sms/submit.jsp?toId="+toId;
			location = url;
    }
  }
...
<form id="smsBodyInfoForm" name="smsBodyInfoForm" method="post">
  ...
  <table class="TableBlock" width="90%" align="center">
    ...
    <tr align="center" class="TableControl">
      <td colspan="2">
        <input type="button" value="发送短信" class="BigButton" onclick="commitSms()">
      </td>
    </tr>
  </table>
</form>
