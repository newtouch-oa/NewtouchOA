...
function toPromptStr(obj){
  ...
  if(obj.remindUrl){
    ...
    remindUrl = contextPath + "/yh/core/funcs/message/act/YHSmsTestAct/acceptedSms.act?pageNo=0&pageSize=20";
  }
 ...
 // var altStr = obj.content.replace(/<br>/g,"");
  ...
  promptStr.push("<DIV class=head><DIV CLASS=moduleHeader><IMG  src=\"" + imgPath + "/sms/sms_type"+ obj.messageType +".gif\" align=absMiddle>&nbsp;&nbsp;" + getSmsTypeDesc(obj.messageType) + "</DIV></div>");
...
function toPanelInBoxStr(obj){
  ...
  if(obj.remindUrl){
    ...
    remindUrl = contextPath + "/yh/core/funcs/message/act/YHMessageTestAct/acceptedMessage.act?pageNo=0&pageSize=20";
  }
  if(obj.remindTime){
  ...
  var contentValue =  obj.content.replace(/[\n\r\f]/g,"");
  var promptStr = "<DIV class=\"module_sms listColor2\" id=module_" + obj.messageBodyId + ">"
    + "<input type=\"hidden\" id=\"deleteFlag\" value=\"" + obj.messageBodyId  + "\">"
    + "<DIV class=head2 ><DIV CLASS=moduleHeader2>"+bindStauts(obj.messageBodyId)+"<span id=\"fromId_" + obj.messageBodyId + "\"><span id='toId_"+obj.fromId+"'> 发给："+getUserName(obj.fromId)+"</span></span>&nbsp;&nbsp;" + timeStr + "</div></div>"
    + "<DIV class=module_body3>"
    + "<DIV class=time><U  style=\"CURSOR: hand\"><input type=\"hidden\" id=\"fromId_" + obj.messageBodyId + "\" value=\"" + obj.fromId + "\"></U>&nbsp;&nbsp;" + "" + " </DIV>"
    + "<DIV class=content>" + contentValue + "</DIV>"
    + "<DIV class=module_page_sms align=right>"
    + "<A href=\"javascript:reSent(" + obj.messageBodyId + "," + obj.fromId + ");\">重发</A>&nbsp;&nbsp;" 
  //  + "<A title=点击查看内容详情 href=\"javascript:Details(\'" + remindUrl + "\',\'" + obj.messageBodyId + "\',0);\">查看详情</A>&nbsp;&nbsp; "
    + "<A title=删除  href=\"javascript:deleteSms(\'" + obj.messageBodyId + "\');\"> 删除</A>&nbsp;&nbsp; "
    + "</DIV></DIV></DIV>";
  return promptStr;
}
function toPanelSentBoxStr(obj){
  ...
  if(obj.remindUrl){
    ...
    remindUrl = contextPath + "/yh/core/funcs/message/act/YHMessageAct/acceptedMessage.act?pageNo=0&pageSize=20";
  }
...
//  var altStr = obj.content.replace(/<br>/g,"");
  ...
  var promptStr =  "<DIV class=\"module_sms listColor2\" id=module_" + obj.bodyId + ">"
    ...
    + "<DIV class=head2><DIV CLASS=moduleHeader2>"+bindStauts(obj.bodyId)+"&nbsp;&nbsp;" +" 发自：" + "<span id='toId_"+obj.bodyId+"'><span id=\"fromId_show" + obj.bodyId + "\">"+getUserName(obj.fromId)+"</span></span>&nbsp;"+timeStr+"</DIV></div>"
    + "<DIV class=module_body3>"
    + "<DIV class=time><U  style=\"CURSOR: hand\">" ;
  if(isShowToId){
    promptStr +=  "<input type=\"hidden\" id=\"fromId_" + obj.bodyId + "\" value=\"" + obj.fromId + "\"><span id=\"fromId_show" + obj.bodyId + "\"></span>";
  }else{
   // promptStr +=  " <a href=\"javascript:showStatus('" + obj.bodyId + "');\">查看</a>";
  }
  promptStr += "</U>&nbsp;&nbsp; </DIV><DIV class=content>" +  contentValue + "</DIV>"
    + "<DIV class=module_page_sms align=right nowrap>";
  /*if(isShowToId){
    promptStr += "<A href=\"javascript:smsSend(\'" + obj.fromId + "\');\">发送</A>&nbsp;" 
      + "<A title=重新发送  href=\'javascript:resetSms(\"" + obj.bodyId + "\",\"" + obj.content.replace(/\"/g,"\\\"") + "\"," + obj.messageType + ",\"" + obj.remindUrl + "\");\'>重发</A>&nbsp;";
   // if(obj.smsType == "0"){
    //  promptStr += "<A title=聊天记录  href=\"javascript:Details(\'" + remindUrl + "\',\'" + obj.bodyId + "\',0);\"> 聊天记录</A>&nbsp;";
   // }
  }*/
  promptStr += "<A title=回复  href=\"javascript:smsSend(" + obj.fromId + ");\"> 回复</A>&nbsp;"
    + "<A title=删除  href=\"javascript:deleteSms(\'" + obj.bodyId + "\');\"> 删除</A>&nbsp;&nbsp</DIV></DIV></DIV>";
  return promptStr;
}
/**
 * 
...
function getSmsTypeDesc(smsType){
  var url = contextPath + "/yh/core/funcs/message/act/YHMessageAct/getMessageTypeDesc.act?";
  var rtJson = getJsonRs(url, "messageType=" + smsType + "&code=SMS_REMIND");
  if(rtJson.rtState == "0"){
...
function markRead(seqId){
  var url = contextPath + "/yh/core/funcs/message/act/YHMessageAct/resetFlag.act";
...
function readAll(){
  ...
  var url = contextPath + "/yh/core/funcs/message/act/YHMessageAct/resetFlagAll.act";
...
function showContent(seqId,type) {
  ...
  var actionUrl = contextPath + "/core/funcs/message/showcontent.jsp?seqId=" + seqId;
  ...
  if (top.YH.version) {
    ...
    openWindow(actionUrl, "查看微讯正文", '360', '160');
  }
...
function bindStauts(smsBodyId){
	  var obj;
	  var html = "";
	  var url = contextPath+"/yh/core/funcs/message/act/YHMessageAct/getStauts.act";
	  var rtJson = getJsonRs(url, "bodyId=" + smsBodyId);
	  if (rtJson.rtState == "0") {
	    obj = rtJson.rtData;
	  }
