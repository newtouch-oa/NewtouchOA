...
//    progress.setStatus("Complete.");
  ...
  function checkForm2(chi) {
    ...
    if (checkForm()) {
      ...
      var url = "";
      if(<%=plan.getSeqId()%>){
        url = contextPath + "/yh/core/funcs/workplan/act/YHPlanWorkAct/updateWork3.act?seqId=<%=plan.getSeqId()%>";
      }
      else{
        url = contextPath + "/yh/core/funcs/workplan/act/YHPlanWorkAct/add.act";
      }
...
//判断是否要显示短信提醒 
  ...
  function doInit3(){
    ...
    if(document.getElementById("dept").value.trim() != null && document.getElementById("dept").value.trim() != "" && document.getElementById("dept").value.trim() != "0" && document.getElementById("dept").value.trim() != "ALL_DEPT"){
      bindDesc([{cntrlId:"dept", dsDef:"DEPARTMENT,SEQ_ID,DEPT_NAME"}]);
    }
   }

  function selectValue(val){
    var otypo = document.getElementById("WORK_TYPE");
  ...
  function deleteAttachBackHand(attachName,attachId,attrchIndex) { 
    ...
    for(var i = 0 ; i < attachNameArrays.length ; i++) { 
      ...
      if (checkForm()) {
        var pars = $('form1').serialize() ;
        var url = contextPath + "/yh/core/funcs/workplan/act/YHPlanWorkAct/updateWork3.act?seqId=<%=plan.getSeqId()%>";
        var json = getJsonRs(url,pars);
        if(json.rtState == "1") {
          alert(json.rtMsrg);
          return false;
...
height="18"><span class="big3">修改工作计划</span></div>
<form name="form1" id="form1" method="post">
<table class="TableBlock"  align="center" width="60%">
<tr>
<td class="TableContent">&nbsp;计划名称：</td>
<td class="TableData"><%if(plan.getName()!=null){ %><input type="text" name="name"
id="name" size="36" maxlength="200" value="<%=plan.getName() %>"><%}%></td>
</tr>
<tr>
<td class="TableContent">&nbsp;发布范围(部门)：</td>
<td class="TableData">
<%if (!YHUtility.isNullorEmpty(plan.getDeptParentDesc())) { %>
<input type="hidden" name="deptParent" id="dept" class="BigInput" size="25" maxlength="25" value="<%=plan.getDeptParentDesc()%>">
<%}else{%>
<input type="hidden" name="deptParent" id="dept" value=''>
<%} %>
<textarea cols="45" name="deptParentDesc" id="deptDesc" rows="2" style="overflow-y: auto;" class="SmallStatic" readonly><%if (!YHUtility.isNullorEmpty(plan.getDeptParentDesc()) && (plan.getDeptParentDesc().equals("0") || plan.getDeptParentDesc().equals("ALL_DEPT"))) {%>
全体部门
<%}%>
</textarea>
...
<%} %>
<textarea cols="45" name="managerDesc"
id="managerDesc" rows="2" style="overflow-y: auto;"
class="SmallStatic"  readonly></textarea> <a
...
<%} %>
<textarea cols="45" name="leader1Desc"
...
<%} %>
<textarea cols="45" name="leader2Desc"
...
<%} %>
<textarea cols="45" name="leader3Desc"
...
<tr id="attr_tr">
      ...
      <td class="TableData">
      <font color='#00AA00'><%if(plan.getAttachmentname()!=null){ %>
      <input type="hidden" id="attachmentName2" name="attachmentName2" value='<%=plan.getAttachmentname() %>'>
      <%} %></font>
      <%if (plan.getAttachmentid()!= null) {%>
        <input type="hidden" id="attachmentId2" name="attachmentId2" value="<%=plan.getAttachmentid()%>">
...
<td class="TableData"><textarea cols=45 id="ATTACHMENT_COMMENT" name="ATTACHMENT_COMMENT" rows=3 class="BigINPUT" ><%if(plan.getAttachmentcomment()!=null){ %><%=plan.getAttachmentcomment()%><%} %></textarea></td>
</tr>
<tr>
<td nowrap class="TableContent">&nbsp;备注：</td>
<td class="TableData"><textarea cols=45 name="remark" rows=3 id="remark" class="BigINPUT" ><%if(plan.getRemark()!=null){%><%=plan.getRemark() %><%}%></textarea></td>
</tr>

<tr>
<td nowrap class="TableContent">&nbsp;有效期：</td>
<td class="TableData">&nbsp;开始日期：&nbsp;<input type="text" id="statrTime" name="statrTime" size="10"
maxlength="19" class="BigInput" value="<%=plan.getStatrTime() %>" readonly> <img
id="beginDateImg" src="<%=imgPath%>/calendar.gif" border="0" style="cursor: hand" align="absMiddle">&nbsp;开始时间不能为空<br>&nbsp;结束日期：
...
border="0" style="cursor: hand" align="absMiddle">&nbsp; 结束时间不能为空
<%} else {%>
<input
type="text" id="endTime" name="endTime" size="10" maxlength="19"
class="BigInput" value="<%=plan.getEndTime() %>" readonly> <img id="endDateImg"
src="<%=imgPath%>/calendar.gif"
border="0" style="cursor: hand" align="absMiddle">&nbsp; 结束时间不能为空
<%} %>
</td>
...
style="display: none" /><input type="hidden" id="CONTENT___Config"
value="" style="display: none" /> <script language=JavaScript>    
       ...
       document.getElementById('DIARY_CONTENT').value = '<%=plan.getContent()%>';
       <%}%>
      </script>
      </div>
</td>
...
<td colspan="2" nowrap><input type="hidden" name="OP" value="">
<input type="hidden" name="PUBLISH" id="PUBLISH" value="1"> <input
type="hidden" name="PLAN_ID" value=""> <input type="hidden"
...
name="ATTACHMENT_NAME_OLD" value=""> <input type="button"
value=" 提交 "  onClick="isUpdateWork()" class="BigButton">&nbsp;&nbsp;
<input type="button" value=" 保存 " class="BigButton" id="baoc" name="baoc" onClick="checkForm2('0')">&nbsp;&nbsp;<input
type="button" value=" 返回 " class="BigButton" onClick="javascript:history.back();"></td>
</tr>
</table>
</form>
