...
<script type="text/javascript" src="<%=contextPath %>/core/funcs/doc/docword/js/wordlogic.js"></script>
<script type="text/javascript">
var treeId = "<%=treeId%>";
var parentStr = "";
function doInit(){
  var url = "<%=contextPath%>/yh/core/funcs/doc/send/act/YHSubjectTermAct/getDept.act?treeId=" + treeId;
...
function commitDept(){
  if(checkForm()&& checkWord()){ 
    var url = "<%=contextPath%>/yh/core/funcs/doc/send/act/YHSubjectTermAct/updateWord.act?treeId=" + treeId;
...
function deleteDept(){
  if(confirm("确认要主题词类别/主题词吗？这将同时删除所有下级类别和和其中的主题词！")){
    var url = "<%=contextPath%>/yh/core/funcs/doc/send/act/YHSubjectTermAct/deleteDept.act?treeId=" + treeId;
    var rtJson = getJsonRs(url, mergeQueryString($("form1"))); 
    if(rtJson.rtState == '0'){
  	  var curTree = parent.wordListTree.tree;
      curTree.removeNode(treeId);
	  window.location.href='<%=contextPath%>/core/funcs/doc/docword/wordInput.jsp';
    }else{ 
	  alert(rtJson.rtMsrg); 
    } 
  }else{
...
function deptFunc(){
  var url = "<%=contextPath%>/yh/core/funcs/doc/send/act/YHSubjectTermAct/selectWordToAttendance.act";
...
function checkWord(){
  var rtJson = getJsonRs(contextPath + "/yh/core/funcs/doc/send/act/YHSubjectTermAct/getAjaxCheck.act", mergeQueryString($("form1")));
  if(rtJson.rtState=="0"){
