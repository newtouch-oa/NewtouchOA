...
function doEdit(seqId){
	location.href = contextPath + moduleContextPath + "/flowrun/documentsType/modify.jsp?seqId=" + seqId;
}
/**
 * 详细信息
 * @param seqId
...
function detail(seqId){
  var URL = contextPath + moduleContextPath + "/flowrun/documentsType/detail.jsp?seqId=" + seqId;
...
function deleteSingle(seqId){
	...
	var requestURLStr = contextPath +moduleSrcPath+"/act/YHDocumentsTypeAct";
...
function loadWindow(flag){
	var url= contextPath + moduleContextPath + "/flowrun/documentsType/docWordIndex.jsp?flag="+flag;
	//var loc_x = document.body.scrollLeft + event.clientX - event.offsetX + 400;
	//var loc_y = document.body.scrollTop + event.clientY - event.offsetY + 400;
	window.showModalDialog(url,self,"edge:raised;scroll:0;status:0;help:0;resizable:1;dialogWidth:500px;dialogHeight:500px;");
}
