...
<%@ page import="yh.core.funcs.person.data.YHPerson,yh.core.global.YHSysProps,yh.core.funcs.doc.act.YHWorkHandleAct" %>
<%
...
function aip_upload()
{
  ...
  if(obj.IsSaved())
    ...
    try{  
        ...
        obj.HttpAddPostString("MODULE","doc");
        obj.HttpAddPostString("ATTACHMENT_ID",dispAipFile);
        URL = "http://<%=host %><%=moduleSrcPath %>/act/YHFlowPrintAct/uploadAipFile.act";
...
function getAipData( flag) {
  ...
  if (nodes) {
    var rtJson = getJsonRs(contextPath + "<%=moduleSrcPath %>/act/YHFlowPrintAct/loadAutoFlag.act", "runId=<%=runId %>&flowId=<%=flowId %>&nodes=" + encodeURIComponent(nodes));
    if (rtJson.rtState == "0") {
      var data=rtJson.rtData;
...
if (!dispAipFile) {
  ...
  success = "1";
  var URL =  "http://<%=host %><%=moduleSrcPath %>/act/YHFlowPrintAct/getAipFile.act?module=workflow&attachmentId=" + dispAipFile;
...
<form style="display:none" action="<%=contextPath %><%=moduleSrcPath %>/act/YHAttachmentAct/uploadFile.act" name="workFlowForm" target="returnPage" id="workFlowForm"  method="post" enctype="multipart/form-data">
...
<iframe id="docIframe" style="margin:0px;padding:0px"  frameborder=0 name="docIframe" width="100%" height="100%" src="<%=contextPath %>/core/funcs/doc/flowrun/list/flowview/viewlist.jsp?runId=<%=runId %>&flowId=<%=flowId %>"></iframe>
</div>
<div id="div-extend1"  style="display:none">
<iframe id="docIframe1" style="margin:0px;padding:0px"  frameborder=0 name="docIframe1" width="100%" height="100%" ></iframe>
</div>
<div id="doc_send_div" style="display:none">
<iframe id="docSendIframe" style="margin:0px;padding:0px"  frameborder=0 name="docSendIframe" width="100%" height="100%" src="<%=contextPath %>/core/funcs/doc/send/send.jsp?runId=<%=runId %>&flowId=<%=flowId %>&prcsId=<%=prcsId %>&flowPrcs=<%=flowPrcs %>"></iframe>
</div>
...
<form  style="display:none" action="<%=contextPath %><%=moduleSrcPath %>/act/YHAttachmentAct/uploadFile.act" name="formFile" target="returnPage" id="formFile"  method="post" enctype="multipart/form-data">
...
<form  style="display:none" action="<%=contextPath %><%=moduleSrcPath %>/act/YHAttachmentAct/uploadFile.act" name="ATTACHMENT1_formFile" target="returnPage" id="ATTACHMENT1_formFile"  method="post" enctype="multipart/form-data">
<input type="hidden"  name="flowId" value="<%=flowId %>"/>
<input type="hidden"  name="runId" value="<%=runId %>"/>
<input type="hidden" name="prcsId" value="<%=prcsId %>"/>
<input type="hidden"  name="flowPrcs" value="<%=flowPrcs %>"/>
 ...
 <%// if (act.getWebsign(request , response)) {  %>
...
<div align="center"><input type="button" class="BigButton" value="返回" onclick="parent.location='<%=contextPath %><%=moduleContextPath %>/flowrun/list/index.jsp?sortId=<%=sortId %>&skin=<%=skin %>'"></div>
</div>
