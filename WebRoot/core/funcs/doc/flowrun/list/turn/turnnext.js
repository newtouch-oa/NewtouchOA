var requestUrl = contextPath +moduleSrcPath+"/act/YHWorkTurnAct";
...
function setTurnNext(str) {
  $('divSmsNex').show();
  $('smsContent').value = str;
...
function setUserSelect( prcsNode ){
  if(prcsNode.prcsId == '0'){
    ...
    $('smsContent').value = prcsNode.docSmsStyle;
      //tooltipMsg1   + runName;
    $('operateButton').onclick = function(){
      if(notAllFinist){
     ...
     // }
      ...
      if(json.rtState == '0'){
        parent.parent.location = "../index.jsp";
      }else{
        alert(json.rtMsrg);
      }
    }
    $('operateButton').value = "结束";
    $('smsContentDiv').hide();
    $('divSmsNex').hide();
    $('need_remind').hide();
  }else{
    ...
    setTurnNext(prcsNode.docSmsStyle);
  }
  //userFilter = prcsNode.userFilter;
}
function showSelectDept() {
  var URL=contextPath + "/core/funcs/doc/send/selectDept.jsp";
  var openWidth = 800;
  var openHeight = 500;
...
function cancelUser(img , id , isOp , prcsId){
  var wi = window['selectUserIframe' + prcsId];
  if(isOp){
    if (wi && wi.outSelected2) {
      wi.outSelected2(id);
    }
...
function checkSendFlag() {
  var url = contextPath + "/yh/core/funcs/doc/send/act/YHDocSendAct/getSendState.act";
...
function TurnNext_forwardPage() {
  if (isManage) {
    ...
    parent.parent.location = "../index.jsp?skin="+skin+"&sortId=" + sortId;
  }
}
/**
 * 当所有步骤没有一个满足条件时
...
function getPrcsText(tmp) {
 ...
 var text = new Array();
 text[0] = '<tr id="userSelectTr_'+i+'" style="display:none" class="TableData"><td style="vertical-align:top">'
 ...
 text[8] =  '</div></td><td id="selectUserTd'+ i +'" style="display:none;" width="520px"><iframe id="selectUserIframe'+ i +'" name="selectUserIframe'+ i +'" width="520px" height="360px" src=""></iframe></td></tr>';
