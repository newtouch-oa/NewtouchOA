...
var actionUrl = contextPath +moduleSrcPath+"/act/YHProcessUserSelectAct";
function getAccord(){
  var data = {
  panel:'left',
...
function getDeptUser (id) {
  $('right').update("");
  var url = contextPath  +moduleSrcPath+"/act/YHFlowProcessAct/getPrivUser.act"
...
function getAllUser() {
  ...
  if (userOp.length > 0) {
    ...
    getRightPal(obj.name , true , false , 0 ,  "此步骤没有设置经办人" );
  }
}
...
function treeNodeClick(id){
  ...
  if(json.rtState == '0'){
    ...
    if (users.length > 0) {
      getRightPal(obj.name , true , true , 0 );
      addUserDiv(users , 0);
      setSelected(User.value.split(','));
    } else {
      getRightPal(obj.name , true , false , 0 );
    }
  }
}
