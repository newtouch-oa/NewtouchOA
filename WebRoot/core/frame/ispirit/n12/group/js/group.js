...
function opts(cellData, recordIndex, columIndex){
	...
	var seqId = this.getCellData(recordIndex,"seqId");
	 ...
	 str = this.getCellData(recordIndex,"groupActive");
	...
	return "<center>"
			+ "<a href=javascript:startSingle(" + seqId + ")>"+text+"</a>&nbsp;&nbsp;&nbsp;"
			+ "<a href=javascript:doEdit(" + seqId + ")>修改</a>&nbsp;&nbsp;&nbsp;"
			+ "<a href=javascript:deleteSingle(" + seqId + ")>删除</a>"
			+ "</center>";
}
function doEdit(seqId){
	location.href = contextPath + "/core/frame/ispirit/n12/group/editGroup.jsp?seqId=" + seqId;
}
/**
 * 详细信息
 * @param seqId
...
function deleteSingle(seqId){
	...
	if(!window.confirm("确认要删除该群 ？")){
		return ;
	}
	var requestURLStr = contextPath + "/yh/core/funcs/system/ispirit/n12/group/act/YHImGroupAct";
	var url = requestURLStr + "/deleteGroup.act";
...
function startSingle(seqId){
	

	var requestURLStr = contextPath + "/yh/core/funcs/system/ispirit/n12/group/act/YHImGroupAct";
	var url = requestURLStr + "/startGroup.act";
	var rtJson = getJsonRs(url, "seqId=" + seqId );
	if (rtJson.rtState == "0") {
		window.location.reload();
	}else {
...
function getStatus(cellData, recordIndex, columIndex){
   var strValue="";
    ...
    str=str.substring(0,1);
	if("1"==str){
		strValue="<font color='green'>已激活</font>";
	}else{
		strValue="<font color='red'>未激活</font>";
	}
	return strValue;
}
