...
<html>
    ...
    <script type="text/javascript" src="crm/js/cus.js"></script>
  	<script language="javascript" type="text/javascript">
		...
		function tabReload(cusState){
			self.location.href="customAction.do?op=toListCustomersByPerson&range=${range}&cusState="+cusState+"&seqId=${seqId}&userName=${userName}";
		}
		function loadFilter(){
			if('${range}'=='1'){
		...
		function loadList(sortCol,isDe,pageSize,curP){
			...
			pars.op = "listCustomersByPersonId";
			pars.range="${range}";
  ...
  <body>
    <div id="mainbox">
    	<div id="contentbox">
        	<div id="title"><a class="refreshButton" href="javascript:void(0)" onClick="self.history.go(0)" title="重新载入页面"></a>&nbsp;CRM管理 > <c:if test="${range!=1}">【<font color="green">${userName }</font>】的</c:if><c:if test="${range==1}">全部</c:if>客户</div>
