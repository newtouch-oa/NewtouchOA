...
function getUserNew(){
  ...
  var deptIdStr = 0;
  person.location = "<%=contextPath%>/crm/customer/person/usernew.jsp?deptId="+deptIdStr;
}

function getQuery(){
  var person = window.parent.contentFrame;
  person.location = "<%=contextPath%>/crm/customer/person/query.jsp";
}
  ...
  if(id.indexOf('r') == -1){
   ...
   if(rtJsons.rtState == '0'){
     if(rtJsons.rtData[0].isPriv){
       parent.contentFrame.location = "<%=contextPath%>/crm/customer/person/usernew.jsp?deptId="+ id;
     }
   }
  }else{   
	id = id.substring(1, id.length);
	parent.contentFrame.location = "<%=contextPath%>/crm/customer/manageAreaSet/privtree.jsp?seqId=" + id + "&userName=" + encodeURIComponent(userName);
  }
}
</script>
</head>
