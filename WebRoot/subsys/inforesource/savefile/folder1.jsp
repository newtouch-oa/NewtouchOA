...
<table border="0" width="100%" cellspacing="0" cellpadding="3" class="small">
  <tr>
    <td class="Big"><img src="<%=imgPath%>/save_file.gif" align="middle"><span class="big3">公共文件柜—批量转存</span>
    </td>
    </tr>
</table>

<form id="form1" name="form1" action="<%=contextPath%>/subsys/inforesource/savefile/folder1Submit.jsp">
<table class="TableBlock" width="90%" align="center">
    ...
    <tr class="TableData" height="60">
      ...
      <%      	
      	if(sortId!=0 || inIt==1){       	    
      %>  
       	<a href="javascript:location.href=requestURL + '/getFolderInfo.act?module=&backFlag=back&seqId=<%=sortId%>&parentId=<%=parentId%>&attachName=' +attachNameEncode + '&attachId= ' "><img src="<%=imgPath%>/parent.gif" align="middle" border="0">返回上级目录</a><br>
      ...
      %>        
	        ...
	        <label for="SORT_<%=sortMap.get("seqId")%>" title="将文件转移至【<%=sortMap.get("sortName") %>】"><%=sortMap.get("sortName") %></label> <a href="javascript:location.href=requestURL + '/getFolderInfo.act?module=&seqId=<%=sortMap.get("seqId")%>&parentId=<%=sortMap.get("sortParent")%>&attachName=' + attachNameEncode + '&attachId= ' " title="进入【<%=sortMap.get("sortName") %>】">下一级</a><br>
