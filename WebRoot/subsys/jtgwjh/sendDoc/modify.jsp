...
function doInit(){
  ...
  if(rtJson.rtState == "0"){
    ...
    stampComplete = data.stampComplete;
    if(data.sendDatetimeShow){
      $('yearShow').value = data.sendDatetimeShow.substring(0,4);
      $('mouthShow').value = data.sendDatetimeShow.substring(5,7);
      $('dayShow').value = data.sendDatetimeShow.substring(8,10);
    }
    if(data.isStamp == 2){
      $('isStampTd').style.display = 'none';
      $('isStampTd2').style.display = '';
      $('isStampNow').checked = true;
    }
	        ...
	        for(var j = 0; j < data2.length; j++){
	          if(data2[j].fileId == attachmentIdArray[i]){
	            ...
	            if(data2[j].fileId == data.mainDocId){
		          ...
		          attachMenuSelfUtilTable("showAttTbody","jtgw",attachmentNameArray[i] ,attachmentIdArray[i], data2[j].fileSize, '','','',selfdefMenu,i,data2[j].reciveDept,data2[j].reciveDeptDesc,checked);
	    ...
	    for(var i = 0; i < data.reciveDeptDesc.length; i++){
	      for(var j = 0; j < data3.length; j++){
	        if(data3[j].reciveDeptDesc == reciveDeptDesc[i]){
			      ...
			      var td2 = $C('td');
			      td2.align = 'right';
			      td2.innerHTML = '<input type="text" id="print_count_'+i+'" name="print_count_'+i+'" maxlength="5" size="5" style="text-align:right;" value="'+data3[j].printCount+'" onchange="consider('+data3.length+')">';
			      countAll = parseInt(countAll) + parseInt(data3[j].printCount);
			      
			      var td3 = $C('td');
			      td3.align = 'right';
			      td3.innerHTML = '<input type="text" id="print_no_start_'+i+'" name="print_no_start_'+i+'" size="5" style="text-align:right;" value="'+data3[j].printNoStart+'" readonly>';
			      
			      var td4 = $C('td');
			      td4.align = 'right';
			      td4.innerHTML = '<input type="text" id="print_no_end_'+i+'" name="print_no_end_'+i+'" size="5" style="text-align:right;" value="'+data3[j].printNoEnd+'" readonly>';
...
function doSubmit3(){
  if(checkForm()){
	  ...
	  addAPINote();
	  $("form1").action = "<%=contextPath%>/yh/subsys/jtgwjh/docSend/act/YHDocSendAct/updateDocInfo.act?seqId=<%=seqId %>&sendFlag=1";
	  $("form1").submit();
  }
}
...
function doSubmit4(){
  ...
  $("form1").action = "<%=contextPath%>/yh/subsys/jtgwjh/docSend/act/YHDocSendAct/updateDocInfo.act?seqId=<%=seqId %>&sendFlag=2";
  $("form1").submit();
}
...
function controlOver(){
  ...
  $('returnControl').onmouseover = function(){
    this.className = "newButton newButtonReturnHover";
  }
  $('returnControl').onmouseout = function(){
    this.className = "newButton newButtonReturn";
  }
...
function isHaveMainDocWord(){
  if($('mainDocId').value && $('wordDocIframe').style.display == 'none' && $('confirmDiv').style.display == 'none'){
    $('tableWordTable').style.display = 'none';
    officeIframe($('mainDocName').value, $('mainDocId').value, "jtgw", 4);
  }
}
...
function officeIframe(attachmentName,attachmentId,moudle,op,signKey,print){
  if(stampComplete != 1){
    ...
    var url = contextPath + "/subsys/jtgwjh/sendDoc/stamp/stampWord.jsp?seqId=<%=seqId%>&flag=1&flagClose=1";
  }
...
function createWord(){
  ...
  if($('createTypeLocal').checked){
    ...
    var json = getJsonRs(url , 'newName='+title+'&seqId=<%=seqId%>');
    if (json.rtState == '0') {
...
<div id="navTabDiv" style="position: fixed;height:40px;background-color: #E5E5E5;top: 5px;width: 100%;">
  ...
  <div id="formControl">
    <input type="button" title="返回" onclick="returnList();" class="newButton newButtonReturn" id="returnControl">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
    <span id="stampButton1" style="display:none;">
      <span id="getStampDiv" style="display:none;"><input type="button" title="获取印章" onclick="getStamp();" class="newButtonB newButtonGet" id="getControl">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>
      <span id="saveStampDiv" style="display:none;"><input type="button" title="盖章确认" onclick="confirmSave();" class="newButtonB newButtonConfirm" id="confirmControl">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>
    </span>
...
<form enctype="multipart/form-data" action="<%=contextPath%>/yh/subsys/jtgwjh/docSend/act/YHDocSendAct/updateDocInfo.act?seqId=<%=seqId %>"  method="post" name="form1" id="form1" onsubmit="">
	<input type="hidden" id="isStamp" name="isStamp">
  ...
  <div align="center"> 
    ...
    <tr style='height:85.3pt'>
      <td width=549 colspan=10 valign=top  class="tabtd6" align="left">
        ...
        <p class=MsoNormal><span  style='color:red'><textarea name="oaCopySend" id="oaCopySend" rows="5" style="" cols="70"></textarea></span></p>
      </td>
    </tr>
   ...
   <tr style='page-break-inside:avoid;height:5.0pt'  >
    ...
    <tr style='height:67.4pt'>
      <td width=549 colspan=10 class="tabtd13">
  ...
  <table class="TableBlock" width="80%" align="center" >
    ...
    <tr height="25">
      ...
      <td class="TableData" colspan="5" >
        <label><input type="radio" id="isSignYes" name="isSign" value="1" onclick="doPrintControl(1);">是</label>&nbsp;
        <label><input type="radio" id="isSignNO" name="isSign" value="0" onclick="doPrintControl(0);" checked>否</label>
...
<form id="form2" name="form2" action="<%=contextPath%>/yh/subsys/jtgwjh/docSend/act/YHDocSendAct/uploadFile.act?seqId=<%=seqId %>" method="post" enctype="multipart/form-data" target="commintFrame">
