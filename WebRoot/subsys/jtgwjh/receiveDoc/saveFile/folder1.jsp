...
var requestURL="<%=contextPath%>/yh/subsys/jtgwjh/docReceive/act/YHJhDocrecvInfoAct";
...
function comeBack(){
  ...
  if(returnSortId!=0){
	  history.back();
		...
		location.href="<%=contextPath %>/subsys/jtgwjh/receiveDoc/saveFile/index.jsp?docReceiveSeqIds=" + docReceiveSeqIds + "&module=<%=module%>&attachId=<%=attachId%>&attachName=" +attachNameEncode;
  }    
...
<form id="form1" name="form1" action="<%=contextPath%>/subsys/jtgwjh/receiveDoc/saveFile/folder1Submit.jsp">
<table class="TableBlock" width="90%" align="center">
    ...
    <tr class="TableData" height="60">
      ...
      <%      	
      	if(sortId!=0 || inIt==1){       	    
      %>  
       	<a href="javascript:location.href=requestURL + '/getFolderInfo.act?module=<%=module %>&docReceiveSeqIds=<%=docReceiveSeqIds %>&backFlag=back&seqId=<%=sortId%>&parentId=<%=parentId%>&attachName=' +attachNameEncode + '&attachId=<%=attachId %> ' "><img src="<%=imgPath%>/parent.gif" align="middle" border="0">返回上级目录</a><br>
    ...
    <tr>
      <td nowrap align="center" class="TableControl">
        ...
        <input type="button"  value="转存归档 " onclick="send();" class="SmallButtonW">&nbsp;&nbsp;
        <%
					}
        %>
        ...
        <input type="hidden" name="docReceiveSeqIds" id="docReceiveSeqIds" value="<%=docReceiveSeqIds%>">
