    ...
    $("form1").appendChild(table);
     for(var i =0 ;i < prcs.length ; i++){
      ...
      if(quizStr != ''){
        ...
        quizStr = quizStr + "<font size='2' color='red'>批注：</font><input type='text' id='memo_"+seqId+"' name='memo_"+seqId+"'>";
      }
...
function getData(){
  ...
  var rtJson = getJsonRs(url, "year=" + yearStr + "&month=" + monthStr); 
  if(rtJson.rtState == "1"){
	alert(rtJson.rtMsrg); 
	return ; 
  }
...
function doInit(checkEnd){
  ...
  var userId = '<%=userId%>';
  var groupId = '<%=groupId%>';
  var url= "<%=contextPath%>/yh/subsys/oa/hr/score/act/YHScoreShowAct/scoreData2.act?checkFlag=" +checkFlag+ "&userId="+userId+"&"+queryParam + "&groupId=" +groupId +"&checkEnd="+checkEnd;
  ...
  var prc = rtJson.rtData;
  if(prc.isSubmit=='1'){
    parent.location = contextPath + "/subsys/oa/hr/score/month/monthEnd/scoreIndex.jsp?userId="+userId+"&groupId="+groupId+"&checkFlag="+checkFlag+"&year=" + year + "&month=" +month +"&currPage=1";
    alert("提交成功！");
  }
}
//得到点击的seqId和答案
