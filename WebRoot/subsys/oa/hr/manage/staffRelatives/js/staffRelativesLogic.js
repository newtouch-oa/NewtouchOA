 ...
 * @return
 */
function relativesItemFunc(cellData, recordIndex, columIndex){
  var classCode = this.getCellData(recordIndex,"relationship");
  var classNo = "HR_STAFF_RELATIVES1";
...
function doEdit(seqId){
	location.href = contextPath + "/subsys/oa/hr/manage/staffRelatives/modify.jsp?seqId=" + seqId;
}
/**
 * 详细信息
 * @param seqId
...
function detail(seqId){
  var URL = contextPath + "/subsys/oa/hr/manage/staffRelatives/detail.jsp?seqId=" + seqId;
...
function deleteSingle(seqId){
	if(!window.confirm("确认要删除该社会关系信息 ？")){
		return ;
	}
	var requestURLStr = contextPath + "/yh/subsys/oa/hr/manage/relatives/act/YHHrStaffRelativesAct";
...
function deleteAll(){
  ...
  if(!idStrs) {
    alert("要删除社会关系信息，请至少选择其中一个。");
    return;
  }
  if(!window.confirm("确认要删除已选中的社会关系信息？")) {
    return ;
  } 
	var requestURLStr = contextPath + "/yh/subsys/oa/hr/manage/relatives/act/YHHrStaffRelativesAct";
	var url = requestURLStr + "/deleteFile.act";
