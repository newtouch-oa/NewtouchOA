...
function sendForm(){
  ...
  if(ATTACHMENT_div){
  	...
  	if(checkForm()){
  		...
  		//alert("smsPerson>>"+smsPerson + "   mobileSmsPerson>>"+mobileSmsPerson);
  		...
  		if(rtJson.rtState == '0'){
  			location="<%=contextPath%>/subsys/oa/confidentialFile/showConfidentialFile/queryFile/folder.jsp?seqId="+seqId;
  	  }else{
  		 	  alert(rtJson.rtMsrg); 
  		}	
  	}
  ...
  if(checkForm()){
    var oEditor = FCKeditorAPI.GetInstance('fileFolder') ;
	  ...
	  if($("ATTACH_DIR").value){
		...
		$("retrunFlag").value = "returnQueryEdit";
	  $("formFile").submit();
  }  
}
...
<table class="TableBlock" width="100%" align="center">
  ...
  <tr align="center" class="TableControl">
    <td colspan="2" nowrap>
      ...
      <input type="button" value="返回" class="BigButton" onClick="location='<%=contextPath%>/subsys/oa/confidentialFile/showConfidentialFile/queryFile/folder.jsp?seqId=<%=seqId %>'">
    </td>
  </tr>
</table>
</form>
...
<form id="newAttachForm" name="newAttachForm" action="" method="post">
	...
	<input id="retrunFlag"	name="retrunFlag" type="hidden" value="returnQueryEdit">   
</form>
</div>
