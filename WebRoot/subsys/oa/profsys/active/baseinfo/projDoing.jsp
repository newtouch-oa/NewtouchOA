...
var pAllTotal = 0;
//项目类别
function getActiveType(selectObj,projActiveType){
  var requestURL = contextPath + "/yh/subsys/oa/profsys/act/YHProjectAct/getCodeItem.act?classNo="+projActiveType; 
...
function doInit() {
  getActiveType($("projActiveType"),"PROJ_ACTIVE_TYPE2");
  //时间
  var parameters = {
...
function selectInProject(){
  pYxTotal = 0;
  pAllTotal = 0;
  var url = "<%=contextPath%>/yh/subsys/oa/profsys/act/active/YHActiveProjectAct/queryActiveProject.act?projType=2&projStatus=0";
   cfgs = {
    dataAction: url,
...
function toOpts(cellData, recordIndex, columInde){
  ...
  return   "<a href='#' onclick='openShowDetilIndex(" + seqId + ");'>详细信息</a> "
           +"<a href='#' onclick='updateStatus(" + seqId + ");'>结束</a><br>"
...
function openNewsIndex(seqId) {
  ...
  window.open("<%=contextPath%>/subsys/oa/profsys/active/baseinfo/news/index.jsp","","status=0,toolbar=no,menubar=no,location=no,scrollbars=yes,resizable=yes,width=950,height=700,left=" + myleft + ",top=50");
}
//详细信息
function openShowDetilIndex(seqId) {
  var myleft = (screen.availWidth - 800)/2;
  window.open("<%=contextPath%>/subsys/oa/profsys/active/baseinfo/news/index.jsp?seqId="+seqId,"","status=0,toolbar=no,menubar=no,location=no,scrollbars=yes,resizable=yes,width=950,height=700,left=" + myleft + ",top=50");
}
//删除记录
