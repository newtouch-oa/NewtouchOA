...
function checkForm() {
  ...
  if ($("perPositionDesc").value == "") {
    alert("职务必填!");
    $("perPositionDesc").focus();
    $("perPositionDesc").select();
    return false;
  }
  if ($("perName").value == "") {
    alert("姓名必填!");
...
function deleteAttachBackHand(attachName,attachId,attrchIndex) { 
  ...
  if (checkForm()) {
    ...
    var requestURL = "<%=contextPath%>/yh/subsys/oa/profsys/source/person/act/YHSourcePersonAct/updatePerson.act";
...
function selectPerson() {
  ...
  if (prc.seqId) {
    ...
    $("perNum").value = prc.perNum;
    $("perPosition").value = prc.perPosition;
    $("perName").value = prc.perName;
    if (prc.perBirthday != "" && prc.perBirthday != null) {
      $("perBirthday").value = prc.perBirthday.substr(0,10);
    }
    if ($("perPosition").value != "") {
      bindDesc([{cntrlId:"perPosition",dsDef:"USER_PRIV,SEQ_ID,PRIV_NAME"}]);
    }
    $("perNation").value = prc.perNation;
    $("perVocation").value = prc.perVocation;
    selectVal(prc.perSex);
    $("orgActive").value = prc.perResume ;
    $("orgNote").value = prc.perNote ;
    $("orgContact").value = prc.perExperience ;
...
//选中
function selectVal(per) {
  var otypo = document.getElementById("perSex");
  for (var i = 0; i < otypo.options.length; i++) {
      if (otypo.options[i].value == per) {
...
function checkForm2() {
  ...
  if (checkForm()) {
    ...
    if (json.rtState == '1') { 
      ...
      window.location.href = "<%=contextPath%>/subsys/oa/profsys/source/person/news/add2.jsp";
    }
  }
}
function checkFCK() {
...
<table border="0" width="90%" cellspacing="0" cellpadding="3" class="small">
  <tr>
    <td>
    <img src="<%=imgPath%>/notify_new.gif" align="absmiddle"><span class="big3"> 修改人士信息</span>&nbsp;&nbsp;
    </td>
  </tr>
</table>
<br>
...
<table class="TableBlock" width="80%" align="center">
    <tr>
      ...
      <input type="hidden" class="BigInput" name="perPosition" id="perPosition" size=10 readonly>
       <textarea cols=20 name="perPositionDesc" id="perPositionDesc" rows=1 class="BigStatic" wrap="yes" readonly></textarea>
         
      <a href="javascript:;" class="orgAdd" onClick="selectRole(['perPosition','perPositionDesc']);">添加</a>
      <a href="javascript:;" class="orgClear" onClick="$('perPosition').value='';$('perPositionDesc').value='';">清空 </a>
      </td>       
    </tr>
    <tr>
