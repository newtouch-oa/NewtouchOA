...
function exportExcel(){
  var url = "<%=contextPath%>/yh/subsys/oa/profsys/act/in/YHInProjectAct/exportXls.act?printStr=" + '<%=printStr%>';
  window.location = url;
}
function doInt() {
  pagesetupNull();
...
<body  style="margin:0;padding:0;" onload="doInt()">
...
<table border="1" bordercolor="#000000" style="border:1px solid #000;margin:auto;width:89%;border-collapse:collapse;">
  <tr height=33>
    ...
    <td width='8%' rowspan="2" align='center' nowrap>来访类别
    </td>
  ...
  <td align="center" nowrap>外办人数</td>
</tr>
<% for (int i = 0; i < list.size(); i++) {
  project = list.get(i);
  String projArriveMMdd = "";
  String projLeaveMMdd = "";
  if(project.getProjArriveTime() != null){
    projArriveMMdd = format.format(project.getProjArriveTime());
  }
  if(project.getProjLeaveTime() != null){
    projLeaveMMdd = format.format(project.getProjLeaveTime());
  }
%>
    <tr height=33>
      <td nowrap align="center"><%=i+1%></td>
       <td nowrap align="center"><%=projArriveMMdd%> 至
  ...
  for(int i = 0; i<countryArray.length ; i++){
    set.add(countryArray[i]);
  }
  String newCountry = "";
 Object[] o = set.toArray();
 for(int i = 0; i<o.length ; i++){
   newCountry = newCountry + o[i]  + "、";
 }
%>
<tr height=33>
