...
<body class="easyui-layout bodySelectNone" id="body" onselectstart="return false">
	...
	<script src="js/strawberry.min-org.js" type="text/javascript" language="javascript"></script>
	...
	<script>
  		...
  		function hOnMouseOver(tagObj, index) {
			...
			jQuery(document).ready(function () {
				...
				var modeTool = global.modeTool;
				console.log(global);
				var id1 = createModeAndSetTitle(global.modeTool,'100','100','images/baseMode2.png','申请');
				var id2 = createModeAndSetTitle(global.modeTool,'100','200','images/baseMode2.png','审批');
				var lineTool = global.lineTool;
				lineTool.create(100,200,1);//画线
				var lineId = 'line'+lineTool.baseLineIdIndex;
				var buildLine = global.lineMap.get(lineId);
				
				var model1 = global.modeMap.get('module'+id1);
				var model2 = global.modeMap.get('module'+id2);
				buildLine.xBaseMode = model1;
				buildLine.wBaseMode = model2;
				buildLine.xIndex = 5;
					buildLine.path="M 153 126 L 199 126";
					buildLine.d="M 153 126 L 199 126";
				
				
			  
				model1.lineMap.put(lineId,buildLine);

              modeTool.changeLineType(buildLine, model1, buildLine);
             model2.lineMap.put(lineId,buildLine);
             modeTool.changeLineType(buildLine, model2, buildLine);
			//	buildLine.xBaseMode = model1;
			//	buildLine.wBaseMode = model2;
			//	line.xIndex = 5;
			//	line.wIndex = 4;
			//	console.log(line);
			//	var d = line.getPath($id('line'+lineTool.baseLineIdIndex))
			//	console.log(line.getLineHead(d));
			//	console.log(line.getLineEnd(d));
			});
			
			function createModeAndSetTitle(modeTool,top, left, imgSrc,titleValue){
				modeTool.create(top,left,imgSrc);
			    var id = modeTool.baseModeIdIndex;//当前Mode的id
				$id('title'+id).innerHTML= titleValue;//设值
				return id;
			}
	</script>
