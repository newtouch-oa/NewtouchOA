...
public class YHAttachmentLogic {
  ...
  public static String COPYPATH = File.separator+"core"+File.separator+"funcs"+File.separator+YHWorkFlowConst.MODULE+File.separator+"workflowUtility";
  ...
  public void delFeedAttachment(int feedId , String attachmentId , String attachmentName , Connection conn) throws Exception{
    //处理数据库
    YHORM orm = new YHORM();
    YHDocFlowRunFeedback fb = (YHDocFlowRunFeedback) orm.loadObjSingle(conn, YHDocFlowRunFeedback.class, feedId);
  ...
  public String getDownPrintPriv(int runId , int flowId , int userId , Connection conn ) throws Exception {
    ...
    String query = "select ATTACH_PRIV from "+ YHWorkFlowConst.FLOW_RUN_PRCS +","+ YHWorkFlowConst.FLOW_PROCESS +" WHERE "+ YHWorkFlowConst.FLOW_RUN_PRCS +".RUN_ID="+ runId +" AND "+ YHWorkFlowConst.FLOW_PROCESS +".FLOW_SEQ_ID="+ flowId +" AND "+ YHWorkFlowConst.FLOW_RUN_PRCS +".FLOW_PRCS="+ YHWorkFlowConst.FLOW_PROCESS +".PRCS_ID AND "+ YHWorkFlowConst.FLOW_RUN_PRCS +".USER_ID="+ userId ;
    Statement stm = null;
    ResultSet rs = null;
    try {
      stm = conn.createStatement();
