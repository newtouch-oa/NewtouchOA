...
import yh.subsys.inforesouce.docmgr.data.YHDocConst;
import yh.subsys.inforesouce.docmgr.data.YHDocNext;
...
public class YHDocReceiveLogic{
  ...
  public List<YHDocReceive> faDocReceive(Connection conn, YHPerson user, int sendStatus, String column, String asc) throws Exception{
    String sql = " select dr.SEQ_ID, dr.DOC_NO, dr.RES_DATE, dr.FROMUNITS, dr.OPPDOC_NO, dr.TITLE, dr.COPIES, dr.CONF_LEVEL, dr.INSTRUCT, dr.SPONSOR, " +
    		                 ...
    		                 " where dr.CREATE_USER_ID = "+ user.getSeqId() +" and dr.SEND_STATUS = " + sendStatus;
   ...
   * @throws Exception 
   */
  public List<YHDocReceive> myReadDocReceive1(Connection conn, YHPerson user, int status ,boolean hasEnd) throws Exception{
    String sql = " select  dr.SEQ_ID, dr.DOC_NO, dr.RES_DATE, dr.FROMUNITS, dr.OPPDOC_NO, dr.TITLE, dr.COPIES, dr.CONF_LEVEL, dr.INSTRUCT, dr.SPONSOR, " +
    ...
    if (!hasEnd) {
      ...
      sql += " and (dr.RUN_ID is not null and dr.RUN_ID != '')";
    }
 ...
 /*   String sql = "update doc_receive set SEND_STATUS = 1, RECE_USER_ID=? where SEQ_ID=?";
    PreparedStatement ps = null;
    try{
  ...
  public Map<String, String> fileUploadLogic(YHFileUploadForm fileForm,
    ...
    try {
      ...
      while (iKeys.hasNext()) {
        ...
        while (YHDiaryUtil.getExist(filePath + "\\" + hard, fileName)) {
          rand = YHDiaryUtil.getRondom();
          fileName = rand + "_" + fileName;
        }
        result.put(hard + "_" + rand, fileNameV);
        fileForm.saveFile(fieldName, filePath + "\\" + YHDocConst.MODULE + "\\" + hard + "\\" + fileName);
      }
    } catch (Exception e) {
      throw e;
    }
...
//多文件上传时用到的----
  ...
  //处理文件
    ...
    String path = YHDocConst.filePath + "\\"  + tmp[0] + "\\" + tmp[1] + "_" + attName;
    File file = new File(path);
    if(file.exists()){
      file.delete();
    } else {
      //兼容老的数据
      String path2 = YHDocConst.filePath + "\\"  + tmp[0] + "\\" + tmp[1] + "." + attName;
   ...
   * @throws Exception 
   */
  public String myDocReceiveJson(Connection conn, YHPerson user, int seqId) throws Exception{
    String sql = " select  dr.SEQ_ID, dr.DOC_NO, dr.RES_DATE, dr.FROMUNITS, dr.OPPDOC_NO, dr.TITLE, dr.COPIES, dr.CONF_LEVEL, dr.INSTRUCT, dr.SPONSOR, " +
                         " dr.RECE_USER_ID, dr.DOC_TYPE, dr.STATUS, dr.SEND_STATUS, dr.CREATE_USER_ID,P.USER_NAME ,dr.ATTACHNAME,dr.ATTACHID, dr.RUN_ID " +
                         " from oa_officialdoc_recv dr, PERSON P  " +
  ...
  public String getFlowId(Connection conn, String runId) throws Exception{
    String sql = "select FLOW_ID, RUN_ID, END_TIME from oa_fl_run where run_id="+ runId;
    PreparedStatement ps = null;
    ResultSet rs = null;
    try{
      ps = conn.prepareStatement(sql);
  ...
  public String getMaxProcsId(Connection conn, String runId)throws Exception{
    String sql = "select max(PRCS_ID) as prcdId from oa_fl_run_prcs where run_id="+ runId;
    PreparedStatement ps = null;
    ResultSet rs = null;
    try{
      ps = conn.prepareStatement(sql);
  ...
  public String getFlowPrcs(Connection conn, String prcsId) throws Exception{
    String sql = "select FLOW_PRCS from oa_fl_run_prcs where PRCS_ID="+ prcsId;
    PreparedStatement ps = null;
    ResultSet rs = null;
    try{
      ps = conn.prepareStatement(sql);
  ...
  public String getPrcsName(Connection conn, String flowPrcs, String flowId)throws Exception{
    String sql = "select PRCS_NAME from oa_fl_process where PRCS_ID = "+ flowPrcs +" and FLOW_SEQ_ID ="+ flowId;
    PreparedStatement ps = null;
    ResultSet rs = null;
    try{
      ps = conn.prepareStatement(sql);
