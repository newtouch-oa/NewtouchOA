...
public class YHProduceReturnApply implements YHIWFHookPlugin {
	...
	public String execute(Connection dbConn, int runId, Map arrayHandler,
		...
		try {
			//更新生产计划的状态
			String sql1 = "update erp_produce_plan set status='"+StaticData.PRODUCE_RETURN_STATUS+"' where id='"+pp_id+"'";
			st.executeUpdate(sql1);
			//更新状态为结束
			String sql = "update erp_produce_bom set status='"+StaticData.OVER+"' where pp_id='"+pp_id+"' and bom_type='2'";
		...
		try{
			if(bomList != null && bomList.size() > 0){
				for(int i=0;i<bomList.size();i++){
						...
						String  dbNum = getDbById(dbConn,map.get("batch").toString(), map.get("pro_id").toString(), map.get("wh_id").toString());
						if(dbNum!=null){
							double num = DoubleCaulUtil.sum(dbNum, map.get("number").toString());
	...
	private List<Map<String,Object>> getBOMList(Connection dbConn,String pp_id)throws Exception{
		...
		try{
			...
			String sql = "SELECT db.id,db.batch,db.wh_id,db.pro_id,db.number FROM erp_produce_bom b LEFT JOIN erp_db_log db ON b.id =db.pod_id WHERE b.pp_id='"+pp_id+"' AND db.flag='"+StaticData.PRODUCE_RETURN+"'";
	...
	private String getDbById(Connection dbConn,String batch,String proId,String whId)throws Exception{
		Statement st  = null;
		ResultSet rs = null;
		try{
			st = dbConn.createStatement();
			String sql = "SELECT num FROM erp_db WHERE pro_id='"+proId+"' and wh_id='"+whId+"' and batch='"+batch+"'";
			rs = st.executeQuery(sql);
			while(rs.next()){
