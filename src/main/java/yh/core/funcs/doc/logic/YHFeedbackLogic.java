...
import yh.core.funcs.doc.util.sort.YHFeedbackComparator;
...
public class YHFeedbackLogic {
   ...
   * @throws Exception
   */
  public void saveFeedback(YHDocFlowRunFeedback feedback , Connection conn) throws Exception{
    YHORM orm = new YHORM();
  ...
  public String getFeedbacksHtml(YHPerson loginUser , int flowId ,int runId , Connection conn) throws Exception{
    ...
    String query = "SELECT * from "+ YHWorkFlowConst.FLOW_RUN_FEEDBACK +" where RUN_ID="+ runId +" order by PRCS_ID,EDIT_TIME";
    ...
    try {
      ...
      while (rs1.next()) {
        ...
        String sPrcsId = String.valueOf(prcsId);
...
//          $CONTENT_VIEW=nl2br($CONTENT_VIEW);//将字符串中的换行符转成HTML的换行符号
  ...
  public List<YHDocFlowRunFeedback> getSignLookFeedback(Map signLookMap , int flowType, int runId  , int userId , Connection conn) throws Exception {
    List<YHDocFlowRunFeedback> list = new ArrayList();
    String query = "select * from "+ YHWorkFlowConst.FLOW_RUN_FEEDBACK +" where RUN_ID =" + runId + " order by EDIT_TIME DESC";
    Statement stm2 = null;
    ResultSet rs2 = null;
    try {
      ...
      while(rs2.next()){
        ...
        if (flowType == 1) { //固定流程
          ...
          String signLookTmp = (String) signLookMap.get(String.valueOf(prcsId));
          if ("2".equals(signLookTmp)) {
            if (!isPrcsUser) {//当登录用户不是该步骤经办人
              seeSign = false ;
            }
        ...
        if (seeSign) {
          YHDocFlowRunFeedback feedback = new YHDocFlowRunFeedback();
          ...
          feedback.setFlowPrcs(rs2.getInt("FLOW_PRCS"));
  ...
  public boolean isOpUser(int prcsId , int runId , int userId , Connection conn) throws Exception{ 
    ...
    String query = "select 1 from "+ YHWorkFlowConst.FLOW_RUN_PRCS +" where RUN_ID =" + runId + " and PRCS_ID=" + prcsId + " and USER_ID=" + userId;
    Statement stm2 = null;
    ResultSet rs2 = null;
    try {
      stm2 = conn.createStatement();
  ...
  public boolean isHoUser(int prcsId , int runId , int userId , Connection conn) throws Exception{ 
    boolean isPrcsUser = false;
    String query = "select 1 from "+ YHWorkFlowConst.FLOW_RUN_PRCS +" where RUN_ID =" + runId + " and PRCS_ID=" + prcsId + " and USER_ID=" + userId + " and OP_FLAG='1'" ;
    Statement stm2 = null;
    ResultSet rs2 = null;
    try {
      stm2 = conn.createStatement();
  ...
  public int getFlowType(Connection conn , int flowId) throws Exception {
    int flowType = 0 ;
    String query = "select FLOW_TYPE from "+ YHWorkFlowConst.FLOW_TYPE +" where SEQ_ID =" + flowId;
    Statement stm2 = null;
    ResultSet rs2 = null;
    try {
      stm2 = conn.createStatement();
  ...
  public void setPrcsMap(Map prcsNameMap , Map signLookMap , int runId , int flowId, int type ,   Connection conn) throws Exception {
    ...
    String queryPrcs = "select PRCS_ID , FLOW_PRCS from "+ YHWorkFlowConst.FLOW_RUN_PRCS +" where RUN_ID=" + runId;
    Statement stm2 = null;
    ResultSet rs2 = null;
    try {
      stm2 = conn.createStatement();
    ...
    for (Map prcs : prcsList) {
      ...
      String prcsStr = String.valueOf(prcsId);
...
//      }
   ...
   * @throws Exception
   */
  public String getFeedbacks(YHPerson loginUser  , int runId , int prcsId , int flowId , int flowPrcs , Connection conn) throws Exception{
    Map prcsNameMap = new HashMap();
    Map signLookMap = new HashMap();
    int type = this.getFlowType(conn, flowId);
    this.setPrcsMap(prcsNameMap, signLookMap, runId, flowId , type, conn);
    List<YHDocFlowRunFeedback> list = this.getSignLookFeedback(signLookMap ,type , runId, loginUser.getSeqId(), conn);
    StringBuffer sb = new StringBuffer();
    for(YHDocFlowRunFeedback tmp : list){
  ...
  public void delFeedback(int seqId ,Connection conn) throws Exception{
    YHORM orm = new YHORM();
    YHDocFlowRunFeedback tmp = (YHDocFlowRunFeedback) orm.loadObjSingle(conn, YHDocFlowRunFeedback.class, seqId);
    String attachmentId = tmp.getAttachmentId() == null ? "" : tmp.getAttachmentId();
    String attachmentName = tmp.getAttachmentName() == null ? "" : tmp.getAttachmentName();
  ...
  public String getSignData(int feedId , Connection conn) throws Exception {
    String signData = "";
    String query = "select SIGN_DATA from "+ YHWorkFlowConst.FLOW_RUN_FEEDBACK +" where SEQ_ID = "+ feedId;
    Statement stm2 = null;
    ResultSet rs2 = null;
    try {
      stm2 = conn.createStatement();
