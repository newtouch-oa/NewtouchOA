  ...
  if(ATTACHMENT_div){
  	...
  	if(checkForm()){
  		...
  		if(rtJson.rtState == '0'){
  	  	...
  	  	//alert("保存成功！");
 			  location.href="<%=contextPath%>/core/funcs/personfolder/folder.jsp?seqId=<%=seqId %>";
  ...
  if(checkForm()){
    var oEditor = FCKeditorAPI.GetInstance('fileFolder') ;
	  ...
	  if($("ATTACH_DIR").value){
		...
		var actionURL = "<%=contextPath%>/yh/core/funcs/personfolder/act/YHPersonFileContentAct/newFileSingleUpload.act?failseFlag=returnEdit&seqId=<%=seqId%>&contentId=<%=contentId %>";
...
function treeSize() { 
  ...
  $("newFileSpan").hide(); 
  $("uploadFileSpan").hide(); 
  $("remindInfo1").innerHTML="个人文件柜容量超过限制 (<%=folderCapacity%> MB)，不能新建附件！ "; 
  $("remindInfo2").innerHTML="个人文件柜容量超过限制 (<%=folderCapacity%> MB)，不能新建附件！ "; 
  //$("formDiv").hide(); 
  //$("returnDiv").show(); 
  flag=0; 
  }else{ 
