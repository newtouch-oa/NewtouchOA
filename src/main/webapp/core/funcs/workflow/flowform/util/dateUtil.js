...
function getIp(){
  var url = contextPath + "/yh/core/funcs/workflow/util/YHFlowFormUtilAct/getIpAdd.act"; 
  var rtJson = getJsonRs(url); 
  return rtJson.rtData.ip;
}
function runName(){ //工作名称/文号
  var url = contextPath + "/yh/core/funcs/workflow/util/YHFlowFormUtilAct/getRunName.act";
  var rtJson = getJsonRs(url);
  if(rtJson.rtState == "0"){
...
function beginTime(){ //流程开始日期
  var url = contextPath + "/yh/core/funcs/workflow/util/YHFlowFormUtilAct/getBeginTime.act";
  var rtJson = getJsonRs(url);
  if(rtJson.rtState == "0"){
...
function runId(){ //流水号
  var url = contextPath + "/yh/core/funcs/workflow/util/YHFlowFormUtilAct/getRunId.act";
  var rtJson = getJsonRs(url);
  if(rtJson.rtState == "0"){
...
function endTime(){ //流程开始日期
  var url = contextPath + "/yh/core/funcs/workflow/util/YHFlowFormUtilAct/getEndTime.act";
  var rtJson = getJsonRs(url);
  if(rtJson.rtState == "0"){
...
function deptNameLong(){//当前用户部门（长名称）
  var url = contextPath + "/yh/core/funcs/workflow/util/YHFlowFormUtilAct/getDeptNameLong.act?";
  var rtJson = getJsonRs(url);
  var result = "";
  if(rtJson.rtState == "0"){
...
function deptNameShort(){//当前用户部门（短名称）
  var url = contextPath + "/yh/core/funcs/workflow/util/YHFlowFormUtilAct/getDeptNameShort.act?";
  var rtJson = getJsonRs(url);
  if(rtJson.rtState == "0"){
...
function getUserPriv(domId){    //权限列表
  var url = contextPath + "/yh/core/funcs/workflow/util/YHFlowFormUtilAct/getSelectData.act";
  var rtJson = getJsonRs(url);
  if(rtJson.rtState == "0"){
...
function getPerson(domId){    //人员列表
  var url = contextPath + "/yh/core/funcs/workflow/util/YHFlowFormUtilAct/getPerson.act";
  var rtJson = getJsonRs(url);
  if(rtJson.rtState == "0"){
  var select = document.getElementById(domId);
  var len = select.options.length;
...
function getDepartment(domId){    //部门列表
  var url = contextPath + "/yh/core/funcs/workflow/util/YHFlowFormUtilAct/getDepartment.act";
  var rtJson = getJsonRs(url);
  if(rtJson.rtState == "0"){
  var select = document.getElementById(domId);
  var len = select.options.length;
...
function deptParent(domId){   //部门主管（上级部门）
  var url = contextPath + "/yh/core/funcs/workflow/util/YHFlowFormUtilAct/getDeptParent.act";
  var rtJson = getJsonRs(url);
  if(rtJson.rtState == "0"){
  var select = document.getElementById(domId);
  var len = select.options.length;
...
function deptLocalInput(){ //部门主管（本部门 input）
  var url =contextPath + "/yh/core/funcs/workflow/util/YHFlowFormUtilAct/getDeptLocalInput.act";
  var rtJson = getJsonRs(url);
  if(rtJson.rtState == "0"){
...
function deptParentInput(){//部门主管（上级部门  input）
  var url = contextPath + "/yh/core/funcs/workflow/util/YHFlowFormUtilAct/getDeptParentInput.act";
  var rtJson = getJsonRs(url);
  if(rtJson.rtState == "0"){
    return rtJson.rtData[0].userName; 
  }else{
...
function deptFirstInput(){ //部门主管（ 一级部门 input）
  var url = contextPath + "/yh/core/funcs/workflow/util/YHFlowFormUtilAct/getDeptFirstInput.act";
  var rtJson = getJsonRs(url);
  if(rtJson.rtState == "0"){
    return rtJson.rtData[0].userName; 
  }else{
...
function deptFirst(domId){   //部门主管（一级部门）
  var url = contextPath + "/yh/core/funcs/workflow/util/YHFlowFormUtilAct/getDeptFirst.act";
  var rtJson = getJsonRs(url);
  if(rtJson.rtState == "0"){
  var select = document.getElementById(domId);
  var len = select.options.length;
...
function deptLocal(domId){ //本部门主管
  ...
  // var url = "/yh/core/funcs/person/act/YHUserPrivAct/getAutoData.act?seqId=" + seqId;
   var url = contextPath + "/yh/core/funcs/workflow/util/YHFlowFormUtilAct/getDeptLocal.act";
   var rtJson = getJsonRs(url);
   if(rtJson.rtState == "0"){
...
function flow_data_picker(obj,item_str)
{
  var id = obj.getAttribute("ID");
...
function data_fetch(obj,run_id,item_str){
  ...
  var URL= contextPath + "/yh/core/funcs/workflow/act/YHFormDataFetch/dataFetch.act";
