...
function openFlowDesign(flowId ) {
  var url = contextPath + "/core/funcs/workflow/flowdesign/index.jsp?flowId=" + flowId;
...
function viewForm(formId) { 
  var url = contextPath + "/core/funcs/workflow/flowform/formView.jsp?seqId=" + formId;
 ...
 * @return
 */
function openFormDesign(formId, id) {
  var url = contextPath + "/core/funcs/workflow/flowform/main2.jsp?seqId=" + formId + "&sortId=" + id;
...
function editProcess(flowId , prcsId) {
  var url = contextPath + "/core/funcs/workflow/flowdesign/viewlist/setproperty/index.jsp?flowId="+flowId;
  var title = "添加步骤";
  if (prcsId) {
    url += "&seqId=" + prcsId;
    title = "修改步骤";
  ...
  //showModalWindow(url , title , "ProcessProperty" ,800,600 , false, function () {location.reload();});
}
 /**
  * 添加或者修改步骤
  ...
  * @return
  */
 function editProcess2(flowId , prcsId) {
   var url = contextPath + "/core/funcs/workflow/flowdesign/index2.jsp?flowId="+flowId;
   if (prcsId) {
     url += "&seqId=" + prcsId;
   }
   loc_x=(screen.availWidth-700)/2;
   loc_y=(screen.availHeight-700)/2;
   window.open(url,"set_process","height=550,width=900,status=1,toolbar=no,menubar=no,location=no,scrollbars=yes,top="+loc_y+",left="+loc_x+",resizable=yes");
   //showModalWindow(url , title , "ProcessProperty" ,800,600 , false, function () {location.reload();});
 }
/**
 * 设置步骤相关项
...
function loadProcessWindow(url , title){
  ...
  //showModalWindow(url , title , "SetProcess" ,800,600 , false, function () {location.reload();});
}
/**
 * 关闭SetProcess窗口
 * @return
...
function viewGraph(flowId) {
   var url = contextPath + "/core/funcs/workflow/flowrun/list/viewgraph/index.jsp?flowId=" + flowId;
   if (window.navigator.appVersion.match(/9./i) =='9.') {
      url = contextPath + "/core/funcs/workflow/flowrun/list/viewgraph/index3.jsp?flowId=" + flowId;
   }
  //showModalWindow(url,'流程图' , "graph" + flowId ,800,600 , false );
 ...
 * @return
 */
function formViewByName(runId , flowName) {
  var url = contextPath + "/yh/core/funcs/workflow/act/YHFlowRunAct/getFlowId.act";
  var  par = 'flowName=' + flowName;
...
function showSign(feedId) {
  var url = contextPath + "/core/funcs/workflow/flowrun/list/signInfo.jsp?feedId=" + feedId;
...
function readAction(){
  ...
  office(attachmentName,attachmentId,"workflow",7);
}
/**
 * 打印
 * @return
...
function printAction() {
  ...
  var attachmentId = down.attachmentId;
  var attachmentName = down.attachmentName;
  office(attachmentName,attachmentId,"workflow",5,"",1);
}
/**
 * 编辑
 * @return
...
function editAction(){
  ...
  var attachmentId = down.attachmentId;
  var attachmentName = down.attachmentName;
  office(attachmentName,attachmentId,"workflow",4);
}
/**
 * 下载
 * @return
...
function downAction(){
  ...
  var attachmentId = down.attachmentId;
  var attachmentName = down.attachmentName;
  downLoadFile(attachmentName ,  attachmentId, "workflow");
}
...
function saveAction(){
  ...
  var attachmentId = down.attachmentId;
  var attachmentName = down.attachmentName;
  archived(attachmentName,attachmentId,"workflow");
}
/**
 * 转存处理函数
 * @param attachName
...
function getSortIdByName(sortName){
  var url = contextPath + "/yh/core/funcs/workflow/act/YHFlowSortAct/getSortId.act";
