...
function loadFlowPrcs(cntrlId){
  ...
  var url = contextPath+'/yh/core/funcs/workflow/act/YHWorkDestroyAct/getFlow.act';
...
function loadFlowType(cntrlId){
  var url = contextPath+'/yh/core/funcs/workflow/act/YHFlowTypeAct/getFlowTypeJsonByManager.act?checkType=1&sortId=' + sortId;
...
function doQuery(cntrlId){
  ...
  location = contextPath + "/core/funcs/workflow/flowrun/destroy/search.jsp?" + param;
}
/**
 * 消息提示
 * @param msrg
...
function destroyBySearch(cntrlId){
  ...
  var param = form.serialize();
  var url = contextPath+'/yh/core/funcs/workflow/act/YHWorkDestroyAct/destroyBysearch.act?' + param;
  var rtJson = getJsonRs(url);
  if(rtJson.rtState == "0"){
    var count = rtJson.rtData;
    location = contextPath + "/core/funcs/workflow/flowrun/destroy/deleteMsrg.jsp?skin="+skin+"&sortId="+ sortId +"&count=" + count;
  }else{
...
function recoverBysearch(cntrlId){
  ...
  var param = form.serialize();
  var url = contextPath+'/yh/core/funcs/workflow/act/YHWorkDestroyAct/recoverBysearch.act?' + param;
  var rtJson = getJsonRs(url);
  if(rtJson.rtState == "0"){
    var count = rtJson.rtData;
    location = contextPath + "/core/funcs/workflow/flowrun/destroy/recoverMsrg.jsp?skin="+skin+"&sortId="+ sortId +"&count=" + count;
  }else{
...
function destroy(){
  ...
  var url = contextPath+'/yh/core/funcs/workflow/act/YHWorkDestroyAct/destroy.act?runId=' + ids;
  var rtJson = getJsonRs(url);
  if(rtJson.rtState == "0"){
    alert(tooltipMsg7);
  }else{
...
function destroyById(runId){
  ...
  var url = contextPath+'/yh/core/funcs/workflow/act/YHWorkDestroyAct/destroy.act?runId=' + runId;
  var rtJson = getJsonRs(url);
  if(rtJson.rtState == "0"){
    location.reload();
  }else{
...
function recover(){
  ...
  var url = contextPath+'/yh/core/funcs/workflow/act/YHWorkDestroyAct/recover.act?runId=' + ids;
  var rtJson = getJsonRs(url);
  if(rtJson.rtState == "0"){
    alert("还原成功！");
  }else{
...
function recoverById(runId){
  ...
  var url = contextPath+'/yh/core/funcs/workflow/act/YHWorkDestroyAct/recover.act?runId=' + runId;
  var rtJson = getJsonRs(url);
  if(rtJson.rtState == "0"){
    location.reload();
  }else{
