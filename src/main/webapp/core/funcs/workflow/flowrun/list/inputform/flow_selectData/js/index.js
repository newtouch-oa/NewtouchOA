...
function doInit() {
  ...
  var dataSrc=selectDataA.getAttribute("DATA_TABLE");
   dataField=selectDataA.getAttribute("DATA_FIELD");
   dataFieldName=selectDataA.getAttribute("DATA_FLD_NAME");
  var dataQuery=selectDataA.getAttribute("DATA_QUERY");
  flowId=selectDataA.getAttribute("flowId");
  flowId = dataSrc;
  //设置sql语句
  //设置列表列  config = this.getConfig(dataFieldName , dataField);
...
function getConfig(fieldName , field) {
  var url =  contextPath + "/yh/core/funcs/workflow/act/YHFormDataSelect/selectFlowData.act";
  var cfgs = {
    dataAction: url,
    container: "container",
  ...
  var s1 = field.split("`");
  colums.push({type:"data", name:'runId'  ,width:150, text:"流水号" , render:centerRender});
  colums.push({type:"data", name:'runName'  ,width:150, text:"文号" , render:centerRender});
  for (var i = 0 ;i < s.length ;i ++) {
    var tmp = s[i];
    if (tmp && tmp != "[文号]" && tmp != "[流水号]") {
      var col = {type:"data", name:'DATA_' + s1[i] ,width:150, text:tmp , render:centerRender};
      colums.push(col);
    }
  }
...
function getParameter(){
  var queryParam = $("form2").serialize() + "&flowId=" + flowId + "&findStr=" + findStr.trim() + "&field=" + dataField.trim()+ "&fieldName=" + dataFieldName.trim();
  return queryParam;
}
/**
 * 操作描画器
