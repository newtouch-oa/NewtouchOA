...
function doInit(){
  ...
  if(type != 3){
    loadData($F('pageIndex') , $F('pageLen') , $F('flowList') , null, "DESC");
  }else{
    loadData($F('pageIndex') , $F('pageLen') , $F('flowList'), null, "DESC");
  }
}
...
function loadFlowType(){
  var url = contextPath+'/yh/core/funcs/workflow/act/YHFlowTypeAct/getFlowTypeJson2.act?sortId=' + sortId;
...
function loadData(pageIndex , showLength , flowId , opFlag , sort){
  ...
  if(type != 3){
    par = "pageIndex="+pageIndex+"&showLength=" +showLength + "&flowId=" + flowId+"&typeStr=" + type + "&sort=" + sort;
  }else{
    par = "pageIndex="+pageIndex+"&showLength=" +showLength + "&flowId=" + flowId+"&typeStr=" + type + "&opFlag=" + opFlag + "&sort=" + sort;
  }
  var date1 = new Date();
  var url =contextPath+'/yh/core/funcs/workflow/act/YHMyWorkAct/getMyWorkList.act?sortId=' + sortId;
...
function addRow(data , i , isLast , title){
  ...
  var runName = "";
  runName = "<a href='javascript:;' onclick='formView2("+ data.runId +"," + data.flowId+","+ data.prcsId +"," +data.flowPrcs +")'>" + data.runName + "</a>";
  var prcsName = "";
  if(type == 3){
  ...
  if(type != 3){
    ...
    var url = contextPath + "/yh/core/funcs/workflow/act/YHFlowExportAct/exportFlowZip.act?runIdStr=" + data.runId;
    operate = "<a href='javascript:' onclick='window.open(\""+ url +"\")'>导出</a>";
    if (data.opFlag == 1 && data.calBackPriv && data.state != "已结束") {
  ...
  if(type != 3){
    ...
    var timeUse2 = "";
    if (data.timeUse) {
      timeUse = "<br><font color=red>"+ data.timeUse + "</font>";
      timeUse2 = "\n" + data.timeUse ;
    }
    td = "<td align=center>" + data.runId +"</td>"
            ...
            + "<td  class=auto title='" + prcsNameTemp + timeUse2 + "'>" + prcsName + timeUse  +"</td>"
            + fields
            + "<td>"+ operate  +"</td>";
  }else{
    var state =  "";
...
function trustAction(){
  ...
  var url = contextPath + "/core/funcs/workflow/flowrun/list/others/"+page+".jsp?"+ sPar;
...
function exportAction(){
  var par = arguments[2]; 
  var url = contextPath + "/yh/core/funcs/workflow/act/YHFlowExportAct/exportFlowZip.act?" + par;
  window.open(url);
}
function delAction(){
...
function sort(flag) {
  var img = "";
  loadData($F('pageIndex') , $F('pageLen') , $F('flowList'), null, flag);
  if (flag == "DESC") {
    flag = "ASC";
    img = "arrow_up.gif";
  } else {
  ...
  $('RUN_ID_TD').onclick = function () {
    sort(flag);
  }
  $("RUN_ID_sortFlagImg").src = imgPath + "/" + img;
}