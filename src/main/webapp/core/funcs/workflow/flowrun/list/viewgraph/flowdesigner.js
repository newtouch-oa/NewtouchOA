...
function createOval(prcs){
	 var r = contextPath+"/yh/core/funcs/workflow/act/YHFlowProcessAct/getGraphSetting.act";
  	 var j = getJsonRs(r , 'graphtype=oval');
  	 var width=120,height=60;
  	 if(j.rtState == '0'){
  	 	width=j.rtData.width;
  	 	height=j.rtData.height;
  	}
  ...
  var sOval = "<oval id="
    ...
    + prcs.leftVml + "; TOP:" 
    + prcs.topVml + ";WIDTH: "+width+"; POSITION: absolute; HEIGHT: "+height+";vertical-align:middle;cursor:pointer;TEXT-ALIGN:center;z-index:1' arcsize='4321f' coordsize='21600,21600' title='▲"
    + prcs.flowTitle + "\n" + prcsToTitle + "'/>"; 
...
function createRoundrect(prcs){
	 var r = contextPath+"/yh/core/funcs/workflow/act/YHFlowProcessAct/getGraphSetting.act";
  	 var j = getJsonRs(r , 'graphtype=roundrect');
...
var r = contextPath+"/yh/core/funcs/workflow/act/YHFlowProcessAct/getGraphSetting.act";
  	 var j = getJsonRs(r , 'graphtype=diamond');
  	 var width=140,height=80;
  	 if(j.rtState == '0'){
  	 	width=j.rtData.width;
  	 	height=j.rtData.height;
  	}
  ...
  var prcsToTitle = "·下一步骤:" + prcsTo;
  var sShapetype = "<shapetype   onDblClick='Edit_Process2("+ prcs.tableId +")'  id='type" + prcs.prcsId + "' coordsize='21600,21600' o:spt='4' path='m10800,l,10800,10800,21600,21600,10800xe'/>";
  ...
  var sShape = "<shape type='#type" + prcs.prcsId + "' id='" + prcs.prcsId + "' table_id='" 
    + prcs.tableId + "' onDblClick='Edit_Process2("+ prcs.tableId +")'  flowType='"
    + prcs.flowType + "' passCount='0'  flowTitle='"
    + prcs.flowTitle + "' flowFlag='0'  readOnly='0' fillcolor='"
    + prcs.fillcolor + "' style='LEFT: "+ prcs.leftVml +"; TOP:"
    + prcs.topVml +"; WIDTH: "+width+"; POSITION: absolute; HEIGHT: "+height+";z-index:1;vertical-align:middle;cursor:pointer;TEXT-ALIGN:center;' title=\"▲"+ prcs.flowTitle + "\n" + prcsToTitle  + prcs.condition.replace(/<br>/gi,'\n') + "\"/>";
  var sShadow = "<shadow on='T' type='single' color='#b3b3b3' offset='3px,3px'/>";
...
function createLine(line){
  ...
  var sStroke = "<stroke endarrow='block'></vml:stroke>";
