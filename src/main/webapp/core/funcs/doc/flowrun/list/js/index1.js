...
function loadFlowType(){
  var url = contextPath+moduleSrcPath+'/act/YHFlowTypeAct/getFlowTypeJson.act?sortId=' + sortId;
...
function loadData(pageIndex , showLength , flowId , opFlag){
  ...
  var date1 = new Date();
  var url =contextPath+moduleSrcPath+'/act/YHMyWorkAct/getMyWorkList.act?sortId=' + sortId;
...
function addRow(data , i , isLast , title){
  ...
  var runName = "";
  runName = "<a href='javascript:;' onclick='formView("+ data.runId +"," + data.flowId +")'>" + data.title + "</a>";
  var prcsName = "";
  if(type == 3){
  ...
  if(type != 3){
    ...
    var url = contextPath +moduleSrcPath+"/act/YHFlowExportAct/exportFlowZip.act?runIdStr=" + data.runId;
...
//            + "<td  class=auto title='" + prcsNameTemp + "'>" + prcsName +"</td>"
    ...
    td = "<td align=center title='" + data.runName + "'>" + runName +"</td>"
            + "<td align=center>" + data.userName +"</td>"
...
function exportAction(){
  var par = arguments[2]; 
  var url = contextPath +moduleSrcPath+"/act/YHFlowExportAct/exportFlowZip.act?" + par;
  window.open(url);
}
function delAction(){
  var runId = arguments[2]; 
...
function deleteRun(runId) {
  if (window.confirm(tooltipMsg5)) {
    var url = contextPath+moduleSrcPath+'/act/YHMyWorkAct/delRun.act';
