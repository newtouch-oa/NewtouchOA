...
<script type="text/Javascript" src="<%=contextPath%>/core/funcs/doc/flowform/util/dateUtil.js" ></script>
<script type="text/Javascript" src="<%=contextPath%>/core/funcs/doc/flowform/util/praserUtil.js" ></script>
...
function selectSign(event)
{
    ...
    var url = contextPath + "/doc/list/inputform/feedHistory.jsp";
    window.open(url,"FEED_HISTORY","status=0,toolbar=no,menubar=no,location=no,scrollbars=yes,resizable=yes,width=300,height=400,left="+loc_x+",top="+loc_y);
}
function checkClose(){
  if (mouse_is_out) {
    if (isNew == 1 ) {
...
function loadAIP(seq_id) {
  var rtJson = getJsonRs( contextPath + "/yh/core/funcs/doc/act/YHFlowPrintAct/loadAip.act", "seq_id="+seq_id);
  if (rtJson.rtState == "0") {
...
function aip_upload()
{
  ...
  if(obj.IsSaved())
    ...
    try{  
        ...
        URL = "http://<%=host %>/yh/core/funcs/doc/act/YHFlowPrintAct/uploadAipFile.act";
...
function getAipData( flag) {
  ...
  if (nodes) {
    var rtJson = getJsonRs(contextPath + "/yh/core/funcs/doc/act/YHFlowPrintAct/loadAutoFlag.act", "runId=<%=runId %>&flowId=<%=flowId %>&nodes=" + encodeURIComponent(nodes));
    if (rtJson.rtState == "0") {
      var data=rtJson.rtData;
...
if (!dispAipFile) {
  ...
  success = "1";
  var URL =  "http://<%=host %>/yh/core/funcs/doc/act/YHFlowPrintAct/getAipFile.act?module=workflow&attachmentId=" + dispAipFile;
...
<form style="display:none" action="<%=contextPath %>/yh/core/funcs/doc/act/YHAttachmentAct/uploadFile.act" name="workFlowForm" target="returnPage" id="workFlowForm"  method="post" enctype="multipart/form-data">
...
<form  style="display:none" action="<%=contextPath %>/yh/core/funcs/doc/act/YHAttachmentAct/uploadFile.act" name="formFile" target="returnPage" id="formFile"  method="post" enctype="multipart/form-data">
...
<form  style="display:none" action="<%=contextPath %>/yh/core/funcs/doc/act/YHAttachmentAct/uploadFile.act" name="ATTACHMENT1_formFile" target="returnPage" id="ATTACHMENT1_formFile"  method="post" enctype="multipart/form-data">
<input type="hidden"  name="flowId" value="<%=flowId %>"/>
<input type="hidden"  name="runId" value="<%=runId %>"/>
<input type="hidden" name="prcsId" value="<%=prcsId %>"/>
<input type="hidden"  name="flowPrcs" value="<%=flowPrcs %>"/>
...
<div align="center"><input type="button" class="BigButton" value="返回" onclick="parent.location='<%=contextPath %>/core/funcs/doc/flowrunRec/list/index.jsp?sortId=<%=sortId %>&skin=<%=skin %>'"></div>
</div>
