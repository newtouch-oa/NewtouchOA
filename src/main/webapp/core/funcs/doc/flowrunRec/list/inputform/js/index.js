var baseContentUrl = contextPath + "/core/funcs/doc/flowrun/list/inputform";
var requestUrl = contextPath + "/yh/core/funcs/doc/act/YHWorkHandleAct";
var isOnloadFinish = false;//是否加载完成
var feedbackUrl = contextPath + "/yh/core/funcs/doc/act/YHFeedbackAct";
...
function useOutline() {
  ...
  var url = contextPath + "/yh/core/funcs/doc/flowrunRec/act/YHMyWorkAct/cancelRun.act";
  var json = getJsonRs(url , 'runId=' + runId + '&flowId=' + flowId +'&prcsId=' + prcsId + '&flowPrcs=' + flowPrcs);
  if(json.rtState == '0'){
    mouse_is_out = false;
    parent.location.href = contextPath + "/core/funcs/doc/flowrunRec/list/inputform/index.jsp?runId="+runId2+"&flowId="+ flowId +"&prcsId=1&flowPrcs=1&isNew=&sortId="+sortId+"&skin=" + skin;
  }
}
function loadSign () {
  if (isIE4 && isHaveSign) {
  ...
  if(isOnloadFinish){
    ...
    if(json.rtState == "0"){
      if(flag == 0){
        ...
        mouse_is_out = false;
        parent.location = contextPath + '/core/funcs/doc/flowrunRec/list/turn/turnnextfree.jsp?skin='+skin+'&sortId='+sortId+'&runId=' + runId + '&flowId=' + flowId +'&prcsId=' + prcsId + '&flowPrcs=' + flowPrcs;
      } else if (flag == 4) {
        savedHandler();
      }
    }else{
...
function turnNextPage() {
  isNew = 0;
  mouse_is_out = false;
  parent.location = contextPath + '/core/funcs/doc/flowrunRec/list/turn/turnnext.jsp?skin='+skin+'&sortId='+sortId+'&runId=' + runId + '&flowId=' + flowId +'&prcsId=' + prcsId + '&flowPrcs=' + flowPrcs;
}
/**
...
function saveAndComeback() {
  isNew = 0;
  mouse_is_out = false;
  parent.location = contextPath + "/core/funcs/doc/flowrunRec/list/index.jsp?sortId=" + sortId + "&skin=" + skin;
}
function finishRunHandler(){
  ...
  var url = contextPath + "/yh/core/funcs/doc/act/YHFeedbackAct/handlerFinish.act";
  var json = getJsonRs(url , 'runId=' + runId + '&flowId=' + flowId +'&prcsId=' + prcsId + '&flowPrcs=' + flowPrcs);
  if(json.rtState == '0'){
    mouse_is_out = false;
    if(json.rtData.turnPage == '0'){
      ...
      mouse_is_out = false;
      parent.location =  contextPath + "/core/funcs/doc/flowrunRec/list/index.jsp?skin="+ skin +"&sortId=" + sortId;
    }
  }
}
/**
...
function editFeedback(event,seqId){
 // top.subWin = window;
  var url = contextPath + "/core/funcs/doc/list/inputform/feedbackedit.jsp?seqId="
    + seqId + '&runId=' 
    + runId + '&flowId=' 
...
function getAttachments(){
  var param = 'runId=' + runId + '&flowId=' + flowId +'&prcsId=' + prcsId +"&flowPrcs=" + flowPrcs;
  var url = contextPath + "/yh/core/funcs/doc/act/YHAttachmentAct/getAttachments.act";
  var json = getJsonRs(url , param);
  if(json.rtState == '0'){
...
function delAction(){
  ...
  var url = contextPath + "/yh/core/funcs/doc/act/YHAttachmentAct/delAttachment.act";
  var json = getJsonRs(url , param);
  if(json.rtState == '0'){
    getAttachments();
  }else{
...
function delAttachAction() {
  ...
  var url = contextPath + "/yh/core/funcs/doc/act/YHAttachmentAct/delFeedbackAttachment.act";
  var json = getJsonRs(url , param);
  if(json.rtState == '0'){
    loadFeedback();
  }else{
...
function newAttach(){
  ...
  var param = 'runId=' + runId 
     ...
     + "&newType=" + $F('newType')
     + "&newName=" + $F('newName');
  var url = contextPath + "/yh/core/funcs/doc/act/YHAttachmentAct/createAttachment.act";
  var json = getJsonRs(url , param);
  if (json.rtState == '0') {
    getAttachments();
...
function back(){
  ...
  if (allowBack == 1) {
    ...
    var url = contextPath + "/core/funcs/doc/list/inputform/goBack.jsp?sortId="+ sortId +"&runId=" + runId + '&flowId=' + flowId +'&prcsId=' + prcsId + "&flowPrcs=" + flowPrcs;
...
function backTo(id , flag){
  ...
  var param = 'runId=' + runId 
      ...
      +"&flowPrcs=" + flowPrcs 
      + "&prcsIdPre=" + id;
    var url = contextPath + "/yh/core/funcs/doc/flowrunRec/act/YHMyWorkAct/backTo.act?sortId=" + sortId;
  var json = getJsonRs(url , param);
  if (!flag) {
    if (json.rtState == '0') {
      mouse_is_out = false;
      parent.location = contextPath + "/core/funcs/doc/flowrunRec/list/index.jsp?sortId=" + sortId;
    } else {
       alert(json.rtMsrg);
    }
  }
...
function updateRunName(){
  ...
  var url = contextPath +  "/yh/core/funcs/doc/act/YHFlowRunAct/updateRunName.act";
  ...
  try {
    ...
    if (sealForm == 1 )  {
      ...
      if(typeof seal_id=="undefined") {
        ...
        var URL = "http://"+ host +"/yh/core/funcs/doc/act/YHFlowFormViewAct/getSeal.act?id=" + seal_id; 
...
function restoreFile(i) {
  ...
  if (i == 1) {
    if (attachmentName) {
      var url = contextPath + "/yh/core/funcs/doc/act/YHAttachmentAct/restoreFile.act?isFeedAttach=true";;
      var json = getJsonRs(url , param);
      if (json.rtState == '0') {
        obj = json.rtData;
        var id = obj.id;
    ...
    if (attachmentName) {
      var url = contextPath + "/yh/core/funcs/doc/act/YHAttachmentAct/restoreFile.act";;
      var json = getJsonRs(url , param);
      getAttachments();
    }
  }
...
function addUser() {
  var url = contextPath + "/core/funcs/doc/list/inputform/addUser.jsp?skin="+skin+"&sortId="+ sortId +"&runId=" + runId + '&flowId=' + flowId +'&prcsId=' + prcsId + "&flowPrcs=" + flowPrcs;
...
function quickLoad(e) {
  ...
  loc_y=event.clientY;
  var url = contextPath + "/core/funcs/doc/list/inputform/quickLoad.jsp";
  window.open( url + "?flowId="+flowId+"&runId="+runId+"&itemId="+itemId+"&itemData=" + itemData,"quick_load","status=0,toolbar=no,menubar=no,location=no,scrollbars=yes,resizable=yes,width=600,height=400,left="+loc_x+",top="+loc_y);
}
