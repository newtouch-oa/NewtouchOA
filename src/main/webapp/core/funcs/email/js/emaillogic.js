...
function sendMail(){
  ...
  var url = contextPath + "/yh/core/funcs/email/act/YHInnerEMailAct/sendMail.act";
  document.form1.boxId.value = "0";
  document.form1.sendFlag.value = "1";
  if(textStr){
    $("form1").content.value = textStr;
  }else{
    $("form1").content.value = "";
  }
  var censorStr1 = censor(textStr,0,1);
 
  var censorStr2 = censor($("form1").subject.value,0,0);
  $("form1").subject.value = censorStr2;
  var sendFlag = "";
  if(censorStr1 == "BANNED" || censorStr2 == "BANNED"  ){
    sendFlag = "0";
  }else if( censorStr1 == "MOD" || censorStr2 == "MOD" ){
    sendFlag = "2";
  }else{
    sendFlag = "1";
  }
