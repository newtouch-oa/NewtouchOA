...
function createOval(node , point , isCenter , isLeft) {
  ...
  textbox.innerHTML =  "<b id='nameText-"+id+"' style='color:#fff'>" + node.nodeName + "</b>";
  textbox2.innerHTML =  "<b id='nameText-"+id+"' style='color:#fff'>" + node.nodeName + "</b>";
...
function createLine(line , relation , name){
  ...
  var text = "<div style='padding-left:3px;color:#000'><b>" + centerNode.nodeName + "," + name + "</b></div><div style='padding-right:3px;padding-left:3px;padding-top:3px;color:#333'>"+relation+"</div>"
...
function drawLine() {
  var centerObj = document.getElementById(centerNode.id);
...
function showTouchGraph( id , url , isLeft , subject) {
  ...
  if (!url) {
    url = contextPath + "/yh/subsys/inforesouce/act/YHTouchGraphAct/getArray.act";
  }
  ...
  var nodeJson = json.rtData;
  centerNode = nodeJson.centerNode;
  ...
  for(var i = 0 ;i < count ; i++){
    var tmp = nodes[i];
    var y = Math.sin(degress * (i + 1)) * tmp.len;
    var x = Math.cos(degress * (i + 1)) * tmp.len;
    y += point.centerY;
    x += point.centerX;
    ...
    var line = {};
    line.toId =tmp.id;
    line.sourceId = centerNode.id ;
    createLine(line , tmp.relation , tmp.nodeName);
  }
