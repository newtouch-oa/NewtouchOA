...
YHJsAttachMenu.prototype = {
  ...
  */
  ...
  getImg: function(extName) {
    ...
    extName = jQuery.trim(extName.toLowerCase());
     if(jQuery.contains(imgArray,extName)){
       return extName;
     }else{
       return "default";
     }
  ...
  attachHtml: function(attachNames, attachIds) {
    ...
    var idArray = attachIds.split(",");
    for (var i = 0; i < nameArray.length; i++) {
      var name = jQuery.trim(nameArray[i]);
      if(!name){
        continue;
      }
      var id = jQuery.trim(idArray[i]);
      var prcsName = "";
      if(this.prec){
        prcsName = i + "_" +  this.prec;
      }else{
      ...
      var extName = "";
      if (index >= 0) {
        extName = jQuery.trim(name.substring(index + 1).toLowerCase());
      }
      ...
      if(jQuery.contains(["doc", "xls", ,"ppt", "docx", "xlsx", "pptx"],extName)){
  ...
  getDocOptType: function(name, readOnly,canDown,selfdefMenu) {
    ...
    if (jQuery.contains(extNames["office"],extName)) {
...
function deleteAttach(dom,cntrlId,extData,j){
  ...
  //删除隐藏字段的值
  deleteAttachment(attachId,attachName);
}
/**
 * 只读offic文档
 * @param dom
...
function deleteFile(attachName,attachId,moudle){
  ...
  var url = contextPath + "/SpringR/fileUpload/deleteFile?" + param;
  jQuery.ajax({
	   type : 'POST',
	   url : url,
	   async:false,
...
function downLoadFile(attachName,attachId,moudle){
  var param = "attachmentName=" + encodeURIComponent(attachName) + "&attachmentId=" + attachId + "&module=" + moudle;
  var url = contextPath + "/SpringR/fileUpload/downFile?" + param;
...
function isMedia(fileName){
   ...
   if (index >= 0) {
     extName = jQuery.trim(fileName.substring(index + 1).toLowerCase());
   }
