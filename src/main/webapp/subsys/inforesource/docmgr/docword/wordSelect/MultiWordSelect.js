...
function getTree(){
  $(defaultIndex).update("");
  var url = contextPath + "/yh/subsys/inforesouce/docmgr/act/YHWordSelectAct/getTree.act?id=";
...
function treeNodeClick(id){
	var url=contextPath+"/yh/subsys/inforesouce/docmgr/act/YHWordSelectAct/getWordBySort.act";
...
function disSelectUser(wordName) {
  ...
  if (Word.tagName == "INPUT") {
    if (Word.value) {
    	Word.value += splitStr + wordName;
    } else {
    	Word.value =  wordName;
    }
  } else {
    if(Word.innerHTML){
    	Word.innerHTML += splitStr + wordName;
    }else{
...
function selectedAll(i){
  ...
  for(var i = 0;i < divs.length ;i++){
    ...
    if(!div.isSelected){
      ...
      wordStr += wordId + splitStr;
    }
  }
  if (wordStr && wordStr.lastIndexOf(splitStr) == wordStr.length - 1) {
...
function getOutofStr(str, s){
  var aStr = str.split(splitStr);
  var strTmp = "";
  var j = 0 ;
  for(var i = 0 ;i < aStr.length ; i++){
    if(aStr[i] && (aStr[i] != s || j != 0)){
      strTmp += aStr[i] + splitStr;
    }else{
      if(aStr[i] == s){
        j = 1;
      }  
  ...
  if (strTmp && strTmp.lastIndexOf(splitStr) == strTmp.length - 1) {
...
function doSearch(value) {
  $('right').update("");
  var url = contextPath+"/yh/subsys/inforesouce/docmgr/act/YHWordSelectAct/getWordBySearch.act";
