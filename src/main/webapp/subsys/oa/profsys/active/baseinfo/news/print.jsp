...
function exportExcel(){
  var url = "<%=contextPath%>/yh/subsys/oa/profsys/act/active/YHActiveProjectAct/exportXls.act?printStr=" + '<%=printStr%>';
  window.location = url;
}
function doInt() {
  pagesetupNull();
...
<% for (int i = 0; i < list.size(); i++) {
  ...
  if(project.getProjStartTime() != null){
    projArriveMMdd = format.format(project.getProjStartTime());
  }
  if(project.getProjEndTime() != null){
    projLeaveMMdd = format.format(project.getProjEndTime());
  }
%>
    <tr height=33>
