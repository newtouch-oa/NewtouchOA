...
function getHrRecruitInfo(){
	var urlStr = "<%=contextPath%>/yh/subsys/oa/hr/recruit/recruitment/act/YHHrRecruitRecruitmentAct";
	var url = urlStr + "/getHrRecruitDetail.act?seqId=<%=seqId%>";
	var rtJson = getJsonRs(url);
	if(rtJson.rtState == "0"){
		var data = rtJson.rtData;
		bindJson2Cntrl(rtJson.rtData);
		if(data.planNo){
			$("planName").value = data.planNo
		}
		if($("assessingOfficer").value.trim()){
			$("assessingOfficerDesc").value = getPersonName(data.assessingOfficer);
		}
		if(data.assPassTime){
			$("assPassTime").value = data.assPassTime.substr(0,10);
		}
		if(data.onBoardingTime){
			$("onBoardingTime").value = data.onBoardingTime.substr(0,10);
		}
		if(data.startingSalaryTime){
			$("startingSalaryTime").value = data.startingSalaryTime.substr(0,10);
		}
	}else{
		alert(rtJson.rtMsrg);
	}
...
function doSubmit(){
	if(checkForm()){
		...
		var urlStr = "<%=contextPath%>/yh/subsys/oa/hr/recruit/recruitment/act/YHHrRecruitRecruitmentAct";
		var url = urlStr + "/updateHrRecruitInfo.act?seqId=<%=seqId%>";
		var rtJson = getJsonRs(url,pars);
		//alert(rsText);
		if(rtJson.rtState == "0"){
			var showInfo = rtJson.rtData.showInfo;
			$("formDiv").hide();
			$("remindDiv").show();
		}else{
			alert(rtJson.rtMsrg); 
...
<table border="0" width="770" cellspacing="0" cellpadding="3" class="small">
  <tr>
    <td><img src="<%=imgPath %>/notify_new.gif" align="middle"><span class="big3"> 编辑招聘计划信息</span>&nbsp;&nbsp;</td>
  </tr>
</table>
...
<table class="TableBlock" width="60%" align="center">
    ...
    <tr align="center" class="TableControl">
      <td colspan=4 nowrap>
        ...
        <input type="button" value="返回" class="BigButton" onclick="location.href='<%=contextPath %>/subsys/oa/hr/recruit/recruitment/manage.jsp';">
      </td>
    </tr>
  </table>
</form>
...
<table class="MessageBox" align="center" width="320">
  <tr>
    <td class="msg info">
      ...
      <div class="content" style="font-size:12pt">招聘录入信息已保存！ </div>
    </td>
  </tr>
</table>
<br><center>
	<input type="button" class="BigButton" value="返回" title="" onclick="location.href='<%=contextPath %>/subsys/oa/hr/recruit/recruitment/manage.jsp';"> &nbsp;&nbsp;
</center>
</div>
</body>
</html>