...
function doEdit(seqId){
	location.href = contextPath + "/subsys/oa/hr/recruit/requirements/modify.jsp?seqId=" + seqId;
}
/**
 * 详细信息
 * @param seqId
...
function detail(seqId){
  var URL = contextPath + "/subsys/oa/hr/recruit/requirements/detail.jsp?seqId=" + seqId;
...
function deleteSingle(seqId){
	if(!window.confirm("确认要删除该招聘需求信息 ？")){
		return ;
	}
	var requestURLStr = contextPath + "/yh/subsys/oa/hr/recruit/requirements/act/YHHrRecruitRequirementsAct";
...
function deleteAll(){
  ...
  if(!idStrs) {
    alert("要删除招聘需求信息，请至少选择其中一个。");
    return;
  }
  if(!window.confirm("确认要删除已选中的招聘需求信息 ？")) {
    return ;
  } 
	var requestURLStr = contextPath + "/yh/subsys/oa/hr/recruit/requirements/act/YHHrRecruitRequirementsAct";
	var url = requestURLStr + "/deleteFile.act";
...
function toExcel(){
  var seqId = checkMags('deleteFlag');

  if(!seqId) {
    alert("要导出招聘需求信息，请至少选择其中一个。");
    return;
  }
  if(!window.confirm("确认要导出已选中的招聘需求信息 ？")) {
    return ;
  } 
  var url= contextPath+ "/yh/subsys/oa/hr/recruit/requirements/act/YHHrRecruitRequirementsAct/toExcel.act?";	
  var param="seqId="+seqId+"&requNo=&requNum=&requJob=&requDeptId=&startTime=&endTime=";
