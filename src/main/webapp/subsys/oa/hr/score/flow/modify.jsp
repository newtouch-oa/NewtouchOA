...
function doInit(){
  ...
  var url = "<%=contextPath%>/yh/subsys/oa/hr/score/act/YHScoreFlowAct/getScoreFlowDetail.act?seqId=${param.seqId}";
  ...
  if(rtJson.rtState == "0"){
    ...
    if($("rankman") && $("rankman").value.trim()){
      bindDesc([{cntrlId:"rankman", dsDef:"PERSON,SEQ_ID,USER_NAME"}]);
    }
  ...
  checkedBox(data.anonymity,data.flowFlag);
  moblieSmsRemind('sms2Remind3','sms2Check');
  getSysRemind();
}

function checkedBox(anonymit,flowFlag){
    if(anonymit == "1"){
      $("anonymityId").checked = true;
    }
    if(flowFlag == "1"){
      $("flowFlagId").checked = true;
    }
}
...
function doSubmit(){
  ...
  if (checkForm()) {
    ...
    var requestURL = "<%=contextPath%>/yh/subsys/oa/hr/score/act/YHScoreFlowAct/updateScoreFlow.act";
...
<table border="0" width="90%" cellspacing="0" cellpadding="3" class="small">
  <tr>
    <td class="Big"><img src="<%=imgPath%>/notify_new.gif" align="absmiddle"><span class="big3"> 编辑考核任务</span>
    </td>
  </tr>
</table>
...
<table width="70%" align="center" class="TableBlock">
    ...
    <tr>
      ...
      <td class="TableData">
        <input type="radio" name="checkStr" id="monthCheck" value="0"  onclick="getRadioValue();"><label for="monthCheck">月考核</label>&nbsp;&nbsp;
...
<tr>
    ...
    <tr align="center" class="TableFooter">
      <td colspan="2" nowrap>
        ...
        <input type="button" value="返回" class="BigButton" onclick="location='<%=contextPath%>/subsys/oa/hr/score/flow/index1.jsp';">&nbsp;&nbsp;
      </td>
    </tr>
  </table>
</form>
