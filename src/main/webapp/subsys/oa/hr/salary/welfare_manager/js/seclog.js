...
function getSecretFlag(parentNo,optDiv,extValue){
	var requestURLStr = contextPath + "/yh/core/funcs/seclog/act/YHSeclogAct/selectChildCode.act?parentNo=" + parentNo;
 ...
 * @return
 */
function secLogFunc(cellData, recordIndex, columIndex){
  var classCode = this.getCellData(recordIndex,"opType");
  var urls = contextPath + "/yh/core/funcs/seclog/act/YHSeclogAct/getSecLogType.act";
  var rtJsons = getJsonRs(urls,"sec_type="+classCode);
  if(rtJsons.rtState == '0'){
    var logName=rtJsons.rtData.logName;
    return  "<center>"+logName+"<center>";
  }else{
    alert(rtJson.rtMsrg);
  }
}
...
function optResult(cellData, recordIndex,columInde){
	var str = "";
	if(cellData){
		if(cellData=="1")
...
function userNameFunc(cellData){
	  var url = contextPath + "/yh/subsys/oa/hr/manage/act/YHHrStaffIncentiveAct/getUserName.act";
	  var rtJson = getJsonRs(url, "userIdStr=" + cellData);
	  if (rtJson.rtState == "0") {
