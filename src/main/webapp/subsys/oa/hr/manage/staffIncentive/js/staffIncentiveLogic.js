 ...
 * @return
 */
function incentiveItemFunc(cellData, recordIndex, columIndex){
  var classCode = this.getCellData(recordIndex,"incentiveItem");
  var classNo = "HR_STAFF_INCENTIVE1";
 ...
 * @return
 */
function incentiveTypeFunc(cellData, recordIndex, columIndex){
	var str = "";
	if(cellData == 1){
		str = "奖励";
	}
 ...
 * @return
 */
function incentiveAmountFunc(cellData, recordIndex, columIndex) {
	var str = "";
	if(cellData){
...
function opts(cellData, recordIndex, columIndex){
	...
	return "<center>"
				+ "<a href=javascript:staffIncentiveDetail(" + seqId + ")>详细信息</a>&nbsp;"
				+ "<a href=javascript:doEdit(" + seqId + ")>修改</a>&nbsp;"
				+ "<a href=javascript:deleteSingle(" + seqId + ")>删除</a>"
				+ "</center>";
}
function doEdit(seqId){
	location.href = contextPath + "/subsys/oa/hr/manage/staffIncentive/modify.jsp?seqId=" + seqId;
}
/**
 * 详细信息
 * @param seqId
 * @return
 */
function staffIncentiveDetail(seqId){
  var URL = contextPath + "/subsys/oa/hr/manage/staffIncentive/incentiveDetail.jsp?seqId=" + seqId;
...
function deleteSingle(seqId){
	if(!window.confirm("确认要删除该奖惩信息 ？")){
		return ;
	}
	var requestURLStr = contextPath + "/yh/subsys/oa/hr/manage/act/YHHrStaffIncentiveAct";
...
function deleteAll(){
  ...
  if(!idStrs) {
    alert("要删除奖惩信息，请至少选择其中一个。");
    return;
  }
  if(!window.confirm("确认要删除已选中的奖惩信息 ？")) {
    return ;
  } 
	var requestURLStr = contextPath + "/yh/subsys/oa/hr/manage/act/YHHrStaffIncentiveAct";
	var url = requestURLStr + "/deleteFile.act";
