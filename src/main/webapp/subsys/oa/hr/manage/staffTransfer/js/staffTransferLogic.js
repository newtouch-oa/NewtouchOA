 ...
 * @return
 */
function transferItemFunc(cellData, recordIndex, columIndex){
  var classCode = this.getCellData(recordIndex,"transferType");
...
function doEdit(seqId){
	location.href = contextPath + "/subsys/oa/hr/manage/staffTransfer/modify.jsp?seqId=" + seqId;
}
/**
 * 详细信息
 * @param seqId
...
function detail(seqId){
  var URL = contextPath + "/subsys/oa/hr/manage/staffTransfer/detail.jsp?seqId=" + seqId;
...
function deleteSingle(seqId){
	if(!window.confirm("确认要删除该人事调动信息 ？")){
		return ;
	}
	var requestURLStr = contextPath + "/yh/subsys/oa/hr/manage/transfer/act/YHHrStaffTransferAct";
...
function deleteAll(){
  ...
  if(!idStrs) {
    alert("要删除人事调动信息，请至少选择其中一个。");
    return;
  }
  if(!window.confirm("确认要删除已选中的人事调动信息 ？")) {
    return ;
  } 
	var requestURLStr = contextPath + "/yh/subsys/oa/hr/manage/transfer/act/YHHrStaffTransferAct";
	var url = requestURLStr + "/deleteFile.act";
