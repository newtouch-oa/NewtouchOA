 ...
 * @return
 */
function reinstatementItemFunc(cellData, recordIndex, columIndex){
  var classCode = this.getCellData(recordIndex,"reappointmentType");
  var classNo = "HR_STAFF_REINSTATEMENT1";
...
function doEdit(seqId){
	location.href = contextPath + "/subsys/oa/hr/manage/staffReinstatement/modify.jsp?seqId=" + seqId;
}
/**
 * 详细信息
 * @param seqId
...
function detail(seqId){
  var URL = contextPath + "/subsys/oa/hr/manage/staffReinstatement/detail.jsp?seqId=" + seqId;
...
function deleteSingle(seqId){
	if(!window.confirm("确认要删除该员工复职信息 ？")){
		return ;
	}
	var requestURLStr = contextPath + "/yh/subsys/oa/hr/manage/reinstatement/act/YHHrStaffReinstatementAct";
...
function deleteAll(){
  ...
  if(!idStrs) {
    alert("要删除员工复职信息，请至少选择其中一个。");
    return;
  }
  if(!window.confirm("确认要删除已选中的员工复职信息 ？")) {
    return ;
  } 
	var requestURLStr = contextPath + "/yh/subsys/oa/hr/manage/reinstatement/act/YHHrStaffReinstatementAct";
	var url = requestURLStr + "/deleteFile.act";
