...
function doInit(){
  ...
  if(rtJson.rtState == "0"){
    ...
    if(data.byEvaluStaffs){
      bindDesc([{cntrlId:"byEvaluStaffs", dsDef:"PERSON,SEQ_ID,USER_NAME"}]);
    }
    if(data.approvePerson){
      bindDesc([{cntrlId:"approvePerson", dsDef:"PERSON,SEQ_ID,USER_NAME"}]);
    }
    if(data.reportTime){
      $("reportTime").value = data.reportTime.substr(0,10);
    }   
    if(data.receiveTime){
      $("receiveTime").value = data.receiveTime.substr(0,10);
    } 
    if(data.approveNextTime){
      $("approveNextTime").value = data.approveNextTime.substr(0,10);
    }
    if(data.startDate){
...
<table border="0" width="100%" cellspacing="0" cellpadding="3" class="small">
  <tr>
    <td><img src="<%=imgPath %>/notify_new.gif" align="middle"><span class="big3"> 编辑职称评定信息</span>&nbsp;&nbsp;
    </td>
  </tr>
</table>
<br>
<form enctype="multipart/form-data" action="<%=contextPath%>/yh/subsys/oa/hr/manage/titleEvaluation/act/YHHrStaffTitleEvaluationAct/updateTitleEvaluationInfo.act"  method="post" name="form1" id="form1" onsubmit="">
  <table class="TableBlock" width="100%" align="center">
    ...
    <tr align="center" class="TableControl">
      <td colspan=4 nowrap>
        ...
        <input type="button" value="返回" class="BigButton" onClick="location='<%=contextPath %>/subsys/oa/hr/manage/staffTitleEvaluation/manage.jsp'">
      </td>
    </tr>
  </table>
</form>
