...
<%
	...
	if(!YHUtility.isNullorEmpty(seqIdStr)){
		tSeqId = Integer.parseInt(seqIdStr);
	}
%>
 ...
 * 初始化信息
 */
function getTrainingPlanInfo(){
	var url = requestURL + "/getTrainingPlanDetail.act?seqId=<%=tSeqId%>";
	...
	var courseStartDate =data.courseStartDate;
	var courseEndDate =data.courseEndDate;
	$("tBcws").value = data.tBcws;
	$("tBcws2").value = insertKiloSplit(data.tBcws,2);
	if(courseStartDate){
		$("courseStartDate").value = courseStartDate.substr(0,10); 
	}else{
		$("courseStartDate").value = "";
	}
	if(courseEndDate){
		$("courseEndDate").value = courseEndDate.substr(0,10); 
	}else{
		$("courseEndDate").value = "";
	}
     ...
     $("attr").innerHTML = "无附件";
  	 ...
  	 $("returnAttName").value = data.attachmentName;
  		var  selfdefMenu = {
  	      ...
  	      img:["downFile","play","dump","deleteFile","insertImg"],  
  	      music:["downFile","play","deleteFile"],  
  		    video:["downFile","play","deleteFile"], 
  		    others:["downFile","deleteFile"]
  			}
  		attachMenuSelfUtil("attr","training",$('returnAttName').value ,$('returnAttId').value, '','','<%=tSeqId%>',selfdefMenu);
   }
	
	fckContentStr = data.tContent
}
...
function deleteAttachBackHand(attachName,attachId,attrchIndex) { 
  var url = requestURL + "/delFloatFile.act?delAttachId=" + attachId + "&seqId=<%=tSeqId%>";
...
function doSubmit(){
    var re1 = /\,/gi;
    $("tBcws").value = $("tBcws2").value.replace(re1,'');
	if(checkForm()){
		var oEditor = FCKeditorAPI.GetInstance('fileFolder');
		...
		document.form1.action = requestURL + "/updateTrainingPlanInfo.act";
		$("form1").submit();
	}
}
...
<table border="0" width="90%" cellspacing="0" cellpadding="3" class="small">
  <tr>
    <td><img src="<%=imgPath %>/notify_new.gif" align="middle"><span class="big3"> 修改培训计划   </span>&nbsp;&nbsp;
    </td>
  </tr>
</table>
<br>
...
<table class="TableBlock" width="60%" align="center">
	<tr>
	  ...
	  <td class="TableData" >
	    <INPUT type="text" name="tPlanNo" id="tPlanNo" maxlength="50" class=BigInput size="12" onblur="checkPlanNo(this.value,'<%=tSeqId %>')">&nbsp;<span id="plan_no_msg"></span>
	  </td>
	   <td nowrap class="TableData">计划名称：<font style='color:red'>*</font></td>
	  <td class="TableData">
  ...
  <tr>
    ...
    <td class="TableData">
     ...
     <td class="TableData">
      ...
      <input type="text" name="chargePersonDesc" id="chargePersonDesc" size="12" readonly class="BigStatic" maxlength="12"  value="">
  ...
  <tr>
    ...
    <td class="TableData" colspan=3>
      <INPUT type="hidden" name="tBcws" id="tBcws" class=BigInput size="12" maxlength="12">
      <INPUT type="text" name="tBcws2" id="tBcws2" class=BigInput size="12" maxlength="12">&nbsp;元
    </td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td class="TableData" colspan=3>
			<span id="smsRemindDiv"><input type="checkbox" name="smsRemind" id="smsRemind" checked><label for="smsRemind">使用内部短信提醒</label>&nbsp;&nbsp;</span>
			<span id="sms2RemindDiv"><input type="checkbox" name="sms2Remind" id="sms2Remind" checked><label for="sms2Remind">使用手机短信提醒</label></span>
		</td>
  </tr>
  <tr id="EDITOR">
  ...
  <tr align="center" class="TableControl">
    <td colspan=4 nowrap>
    	...
    	<input type="hidden" id="seqId" name="seqId" value="<%=tSeqId %>">
