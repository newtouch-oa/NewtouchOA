	 ...
	 var ATTACHMENT_div=$("ATTACHMENT_div").innerHTML;
	  if(ATTACHMENT_div){
	  	...
	  	if(checkForm()){
	  		...
	  		var url = requestURL + "/addNewFileInfo.act?seqId="+ seqId + "&fileType=" + fileType +"&contentId=<%=contentId%>&smsPerson="+smsPerson + "&mobileSmsPerson=" + mobileSmsPerson + "&folderPath=" + encodeURIComponent(folderPath);
	  		var rtJson=getJsonRs(url,pars);
	  		if(rtJson.rtState == '0'){
	  	  	location="<%=contextPath%>/subsys/oa/confidentialFile/showConfidentialFile/fileRegister/folder.jsp?seqId="+seqId;
	  	  }else{
	  		 	  alert(rtJson.rtMsrg); 
	  		}	
	  	}
...
function newAttach(){
  var oEditor = FCKeditorAPI.GetInstance('fileFolder') ;
	...
	if(checkForm()){
		...
		location.href = "<%=contextPath%>/yh/subsys/oa/confidentialFile/act/YHConfidentialContentAct/createFile.act?" + pars + "&seqId=<%=seqId%>&contentId=<%=contentId%>";
...
<div>
<form id="formFile"	action="<%=contextPath%>/yh/subsys/oa/confidentialFile/act/YHConfidentialContentAct/newFileSingleUpload.act?seqId=<%=seqId%>&contentId=<%=contentId%>"
