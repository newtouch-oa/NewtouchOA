  ...
  var url = "<%=contextPath%>/yh/subsys/oa/giftProduct/instock/act/YHGiftInstockAct/queryAllGift.act";
   cfgs = {
    ...
    colums: [
       ...
       {type:"hidden", name:"deptId", text:"上缴部门",align:"center", width: "8%"},
       {type:"text", name:"deptIdDesc", text:"上缴部门",align:"center", width: "8%"},
...
function toOpts(cellData, recordIndex, columInde){
  ...
  var seqId = this.getCellData(recordIndex,"seqId");
  var giftName = this.getCellData(recordIndex,"giftName");
  var giftQty = this.getCellData(recordIndex,"giftQty");
  var re1 = /\'/gi;
  var re2 = /\"/gi;
  var re3 = /\!/gi;
  var re4 = / /gi;
  giftName = giftName.replace(re1,"&lsquo;");
  ...
  if(parseInt(giftQty,10)>0){
    return "<a href=\"#\"  onclick=createNewWork('礼品领用','礼品ID="+seqId
    +"&礼品名称="+giftName
    +"&礼品现存="+giftQty
    +"');> 申领礼品</a>";////general/workflow/new1/edit.php?FLOW_NAME=%
  }
  return "";
...
function insertTr(){
  ...
  mynewcell.innerHTML = "<label for='allbox_for'>全选</label> &nbsp;<a href='javascript:delete_arrang();' title='删除所选礼品'>"
      + "<img src='<%=imgPath%>/delete.gif' align='absMiddle'/>&nbsp;删除所选礼品</a>&nbsp";
}
</SCRIPT>
</head>
